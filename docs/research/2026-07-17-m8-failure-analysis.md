...
대비 무엇을 구제하고 무엇을 못 구제했는지는 `docs/baselines.md`의 "M8 32K
민감도" 소절(기존 관찰)을 그대로 인용할 뿐, 그 기제(축약이 덜 일어나서/
프롬프트가 안 잘려서 등)는 이 노트에서 추정하지 않는다.
...
5. **컨텍스트 오버플로 내성 4건(스펙 §8 이월)** — 이번 3배치에서 오버플로로
   ...
   — 이 절 서두의 관측 한계 원칙대로 "압박은 있었지만 안 보였을 뿐"이라는
   해석은 하지 않는다(증거가 없다는 뜻이지, 없었다는 뜻이 아니다).
...
4. **베이스 테스트 doc 주석의 평가 메타 어휘 누출(최종 브랜치 리뷰에서 본
   분석 이후 발견)**: 이 노트 작성 시점에는 몰랐지만, tasks-large 베이스
   테스트(`*_basic.rs`)의 doc 주석이 "금지 구역"/"판정 신호"/"~을 심는다"
   같은 평가자 어휘를 그대로 담고 있어 grep으로 노출되고 있었다(2026-07-17
   리워드 커밋에서 인다월드 어휘로 교체 — `tasks-large/README.md` "모델
   비노출 원칙" 절 참고). 27런 중 10런이 이 주석을 grep 결과에서 봤다(5
   gemma@8K, 2 ornith@8K, 3 ornith@32K update-vat) — 즉 해당 런들은 정답
   파일 목록의 일부를 답안 채점 어휘와 함께 간접적으로 얻을 수 있는 상태였다.
   특히 §2의 "**#2 (v1/v2 오인 유도)**: … 전부 저항" 귀속은 fix-monthly-total
   주석을 본 gemma 런들에 대해서는 부분적으로 교란됐을 수 있다 — 저항이
   함정 설계의 견고성 때문인지, 주석이 "이 값은 절대 단정하지 않는다(과제1의
   심은 버그 …)"처럼 무엇을 보지 말아야 하는지를 직접 알려줬기 때문인지
   분리할 수 없다. 이 노트의 다른 결론(특히 §3 find-definition-large의
   "grep 한 방" 서사, #7/#8/#9 관찰)은 코드/주석 자체가 아니라 함수·상수
   본체를 대상으로 한 것이라 이 누출과 직접 얽히지 않는 것으로 보이지만,
   확실히 배제하지는 못한다. M9 측정은 리워드된 픽스처로 다시 수행해야
   한다.
