...
승인일: 2026-07-17. 브레인스토밍으로 확정, 스펙 전문 리뷰 2라운드 반영(1R:
Important 3·Minor 6·Nit 3, 2R: Minor 2·Nit 4 — 전건, 코드 실측 대조 리뷰,
2R 판정 Ready=Yes). 직전 상태: M8 완료(main `86882f8`,
...
카운트는 "summary 없는 finish"의 연속 횟수 — **디스패치되거나 게이트에서
거부된** 다른 액션이나 유효 finish가 오면 리셋하고, 액션 없는 턴(파싱 실패
재시도·length-cut)은 증가도 리셋도 없이 유지한다. 기존 FINISH_ERR 에코·반복 계수 편입(M5 §7.3)은 불변 —
...
`cat`/`grep` 7~9회 재반복)다. 런당 상태 `mutated`·`armed`·`idle_count`(카운트된
턴의 반복-호출 여부 이력 포함)·래치로 이벤트 구동 상태기계를 정의한다. 여기서 "뮤테이션"은 **edit_file/write_file의
...
| 그 외 디스패치·거부 턴(미지 도구 호출, 게이트 거부된 run_command) | 상태·카운터 불변 — 오류/노이즈 턴은 BadArgs 에코·스트릭 교정이 전담하고, 재검증 신호를 흐리지 않는다 |

**발동 조건**: `armed` ∧ `idle_count ≥ 4` ∧ 카운트된 최근 4턴 중 **1턴 이상이
반복 호출**(반복 윈도에 동일 (도구|인자) 키가 이미 존재하는 호출) ∧ 미래치 →
1회 주입 후 런당 래치. 주입은 M5 관례대로 발동 턴의 tool_result 본문에 병합.
같은 턴에 반복정지(5회째 동일 히트)와 충돌하면 **반복정지가 우선**한다 — 순수
동일-명령 루프는 순환 교정(3회째)·반복정지(5회째)가 전담하고, FINISH_NUDGE는
이종/혼합 재검증 루프(관측된 r1·r2형: `cargo test`로 무장 후 `cat`/`grep`
변주)를 겨냥한다.
...
- `agent/mod.rs` Scripted 루프: ① finish({}) 2연속 → FINISH_ARGS_CORRECTION
  ...
  조건) ⑥ 무효 finish 시도가 4-2 카운터를 리셋 ⑦ 무액션 턴은 두 카운터 모두
  불변 ⑧ 순수 동일-명령 루프에서 니지와 5회째가 겹치면 반복정지 우선
