...
| Modify `src/eval/sandbox.rs` | `copy_tree` `pub(crate)` 승격 |
...
- Modify: `src/eval/mod.rs:4-6` (모듈 선언), `src/eval/mod.rs:271,280,309` (헬퍼 가시성), `src/eval/sandbox.rs:69` (copy_tree 가시성)
- Test: `src/eval/verify.rs` 내 `#[cfg(test)]` 모듈

**Interfaces:**
- Consumes: `super::task::load_tasks(&Path) -> anyhow::Result<Vec<Task>>` (이름순 정렬 보장), `super::sandbox::{Sandbox, copy_tree}`, `super::{cargo_tripwire, scaled_timeout, with_implicit_protected}`, `crate::tools::exec::{exec_shell, ExecEnd}` (`exec_shell(command: &str, cwd: &Path, timeout: Duration, cancel: &AtomicBool) -> std::io::Result<Exec>`)
- Produces: `pub struct VerifyOptions { pub tasks_dir: PathBuf, pub timeout_scale: f64 }`, `pub struct VerifyRecord { pub name: String, pub discriminates: bool, pub solvable: bool, pub error: Option<String> }` + `impl VerifyRecord { pub fn ok(&self) -> bool }`, `pub async fn run_verify(opts: &VerifyOptions) -> anyhow::Result<Vec<VerifyRecord>>`, `pub fn render_verify_table(records: &[VerifyRecord]) -> String` — Task 2(main 배선)가 이 넷을 사용

- [ ] **Step 1: mod.rs·sandbox.rs 가시성 준비**
...
`src/eval/sandbox.rs:69`의 `fn copy_tree(src: &Path, dst: &Path) -> anyhow::Result<()>`도 `pub(crate) fn`으로. 이 시점엔 verify.rs가 없어 컴파일이 깨지므로 Step 2와 함께 진행한다.

- [ ] **Step 2: verify.rs 뼈대 + 실패하는 테스트 작성**
...
use super::sandbox::{copy_tree, Sandbox};
...
이어서 같은 파일 하단에 테스트 모듈(check 실행이 `sh -c` 의존이라 기존 eval 테스트와 동일하게 unix 게이트):
...
Expected: `todo!()` 패닉으로 전 테스트 FAIL (컴파일은 성공해야 한다 — 컴파일 에러면 가시성 승격 누락)
    ...
    fn walk(base: &Path, dir: &Path, protected: &[String]) -> anyhow::Result<Option<String>> {
        for entry in std::fs::read_dir(dir)? {
            ...
            if entry.file_type()?.is_dir() {
                if let Some(hit) = walk(base, &path, protected)? {
                    return Ok(Some(hit));
                }
            }
        }
...
git commit -m "feat(eval): 판정기 메타테스트 코어 — 변별성·해결가능성 검증 (M6 §4)"
```
...
Expected: `검증 12/12 통과`, exit=0 — **정규형 솔루션은 수선 전 현행 판정기로도 통과가 기대치다** (스펙 §8 1단계). ✗ 항목이 나오면 그것이 감사 발견(솔루션 저작 오류 또는 변별성 회귀)이므로 해당 항목을 수정하고 재실행한다. 발견 내역은 커밋 메시지에 기록.
...
```bash
git add tasks/find-definition tasks/count-usages tasks/fix-off-by-one
...
- fix-off-by-one: 비변별 `zero` 테스트를 변별 케이스로 교체
- (Task 4 감사에서 추가 수선이 있으면 여기 추가)
