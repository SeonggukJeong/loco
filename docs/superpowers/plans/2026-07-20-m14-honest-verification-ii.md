...
**실제 시그니처를 확인했다**(플랜 리뷰 실측): `Agent::new(client, registry, ctx, model, config)`
**5인자이고 `Box`가 아니다.** 진입점은 `run_quiet(&mut agent, &mut session, request)`이며
**`run_with_session`은 코드베이스에 존재하지 않는다.** 헬퍼는 `agent/mod.rs`의
`make_guided_agent`(`:721`)·`new_session`(`:730`)·`run_quiet`(`:734`)를 쓸 것 —
...
**Step 7 구현 후 반드시 뮤테이션 검사를 돌릴 것**(T7 Step 5-a와 같은 규율):
`session.record_extra(...)`를 `session.push(ChatMessage::assistant(...))`로 되돌리면
**단언 ①이 실패해야 한다.** 실패하지 않으면 blob이 너무 작아 `pack()`의 쌍 삭제가
발동하지 않는 것이다 — blob 크기를 올릴 것. 마찬가지로 `push_recovery_notice`를
`push_user_request`로 되돌리면 **④가 사본 5로 실패해야 한다**(플랜 리뷰 실측).
**두 뮤테이션이 각각 실패를 만들지 않으면 이 테스트는 기함 기준의 핀이 아니다.**
...
Run: `cargo test && cargo clippy --all-targets -- -D warnings`
Expected: 전건 통과. **회복 문구가 히스토리에 남는지를 단언하는 기존 테스트가 있다**
(`messages()`에서 `"cut off"`를 찾는 것). 병합 경로에서도 남으므로 통과해야 한다 —
깨지면 그 테스트가 무엇을 고정하는지 읽고 판단할 것. **줄번호로 찾지 말 것**:
이 태스크가 같은 파일을 이미 고쳐 줄이 밀렸다. `grep -n "cut off" src/agent/mod.rs`로
찾을 것
...
**Files:**
- Modify: `src/llm/types.rs:38-70`, `src/agent/mod.rs` (T2가 고친 length 분기의 `record_extra` 한 줄), **`src/eval/mod.rs`**
- Test: `src/llm/types.rs` tests

**⚠ `src/eval/mod.rs`가 Files에 있는 이유**: `#[serde(default)]`는 **역직렬화만**
구제하고 **구조체 리터럴은 전 필드를 요구한다.** Step 3을 적용하면 정확히 **4곳**이
깨진다(플랜 리뷰 실측): `agent/mod.rs:693`(`usage`)·`:695`(`reasoning_content`),
`eval/mod.rs:427`(`usage`)·`:429`(`reasoning_content`). **이 파일을 커밋에서
빠뜨리면 깨진 파일이 커밋 밖에 남는다.**
...
`Agent::new(&script, Registry::guided(), ToolCtx::new(root), "m".into(), &config)` 5인자,
`run_quiet(&mut agent, &mut session, "TASK")`. `run_with_session`은 존재하지 않는다.
...
```bash
git add src/llm/types.rs src/agent/mod.rs src/eval/mod.rs
...
**의도적 편차 — 플랜 리뷰가 수용했다.** 스펙 §3-1/§3-4는 *"명령 문자열을 받도록
시그니처를 넓히고"*라고 쓰지만, **스펙 §3-3의 구현 제약이 이미 반대 방향을
지시한다**: *"별도 `is_piped` 값을 도입하고 두 소비 지점이 각각 판단한다."*
`has_unquoted_pipe`를 두 곳에서 호출하면 M12가 확립한 "배선 지점에서 1회 파싱해
소비자들이 공유한다"는 계약도 깨진다. **따라서 스펙 위반이 아니라 스펙 준수다** —
다음 리뷰어가 재논쟁하지 않도록 근거를 남긴다.
...
fn rule_4_falls_back_to_rule_5_when_the_output_came_through_a_pipe() {
    ...
    let note = n.on_turn(&ctx(3, false, true)).unwrap();
...
fn rule_4_still_renders_all_passed_without_a_pipe() {
    ...
    assert!(n.on_turn(&ctx(3, false, true)).unwrap().contains("all 8 passed"));
}
...
**`ctx` 헬퍼를 3인자로 명시한다** — 2인자로 두면 `mutation_ok`가 `false`로 고정되기
쉽고, 그러면 Step 6의 행렬에서 **뮤테이션 분기 60상태가 통째로 사라져** 단언이
실패한다(플랜 리뷰 실측: `mutation_ok=false` 고정 → 60/120 실패 / `muts` 연동 →
120/120 통과):

```rust
    fn ctx(turn: usize, mutated_since_verify: bool, mutation_ok: bool) -> status_note::TurnCtx {
        status_note::TurnCtx {
            turn,
            max_turns: 25,
            mutation_ok,
...
위 네 테스트는 `record_mutation` 후이므로 `ctx(3, false, true)`로 부른다.
...
fn every_rendered_status_line_keeps_the_block_contract() {
            ...
            for s in &summaries {
                for e in &exits {
                    for piped in [false, true] {
                        ...
                        let Some(note) = n.on_turn(&ctx(5, since, muts)) else { continue };
                        rendered += 1;
    ...
    // 플랜 리뷰 실측: mutation_ok를 muts에 연동하면 120/120
    assert_eq!(
        rendered, expected,
...
**⚠ 문자열 접두로 헤더를 거르지 말 것.** 초판은 `!l.starts_with("---")`로 파일
헤더를 걸렀는데, unified diff에서 **`--`로 시작하는 줄을 삭제하면 `---flag`가 되어
그 필터에 걸린다.** 삭제 줄이 본문과 카운트 양쪽에서 사라져 한 줄을 지운 편집이
`-0 lines, +0 lines`로 렌더된다(플랜 리뷰가 재현). **A-3의 존재 이유인 "조용한
삭제 가시화"를 정면으로 뒤집고, ±3줄 컨텍스트가 있던 이전보다 오히려 후퇴한다.**
파일럿 대상 레포가 전부 CLI 도구라 `--flag`가 clap 정의·테스트·문서에 흔하고
마크다운/YAML `---` 구분자도 같은 경로를 탄다.

**`similar`의 구조화 API를 쓴다 — 분류가 문자열 추측이 아니라 타입으로 성립한다.**
아래 구현은 5개 케이스로 실행 검증됐다(`--flag` 삭제 / `++x` 추가 / 소규모 편집
순서 보존 / 21줄 삭제 상한 / 절단 시 삭제 우선).
...
> **⚠ 이 테스트를 "실패하게" 만들려고 이벤트 매핑을 건드리지 말 것 — 두 곳 모두.**
> ① `finish_nudge`의 이벤트 처리(`VerifyOk`/`Other`/`MutationAttempt`)
> ② **거부 경로의 이벤트 구성**(`agent/mod.rs`의
> `"edit_file" | "write_file" => MutationAttempt`, `_ => Other`)
>
> ①은 **스펙 §3-3-3이 실측으로 기각했다**(`VerifyOk` 매핑에 손대면 재검증 루프
> 감지가 죽는다). ②는 `armed`가 살아남게 만드는 우회로이고 같은 결과를 낳는다.
> 5라운드 스펙 리뷰가 세운 결론을 구현 단계에서 되돌리는 유일한 경로가 이것이다.
