...
`src/eval/mod.rs`의 `#[cfg(test)] mod tests`에 추가 (동기 함수라 `#[test]`):
...
`src/eval/mod.rs` tests에 추가. 기존 eval 통합 테스트들은 `#[cfg(unix)]`지만 이 둘은 파일시스템만 쓰므로 게이트 불필요:
...
fn build_turn(v: serde_json::Value) -> Result<ModelTurn, String> {
    ...
    let mut merge = |map: &mut serde_json::Map<String, serde_json::Value>, k: String, val: serde_json::Value, salvaged: &mut bool| match val {
    ...
    for (k, val) in act {
        merge(&mut map, k, val, &mut salvaged);
    }
    for (k, val) in top {
...
(클로저의 가변 차용이 clippy/차용 검사에 걸리면 `merge`를 일반 `fn merge_entry(map: &mut serde_json::Map<...>, k: String, val: Value) -> bool` 자유 함수로 빼서 `salvaged |= merge_entry(...)` 형태로)
...
Run: `cargo test && cargo clippy --all-targets -- -D warnings`
Expected: 전체 PASS (grep 기존 테스트 포함)
...
사용자에게 알림: "Batch 1 측정 준비 — LM Studio에 qwen3-vl-4b를 ctx 8192로 단독 로드해 주세요." 확인 방법:
...
fn replace_all_applies_per_location_indent_at_stage_three() {
    // 두 위치의 들여쓰기가 다름 — 각자 자기 indent (스펙 §6.4)
    let (dir, ctx) = setup("fn a() {\n    do_it();\n}\nfn b() {\n        do_it();\n}\n");
    let out = EditFile
        .run(&serde_json::json!({"path": "f.rs", "search": "do_it();", "replace": "do_other();", "replace_all": true}), &ctx)
        .unwrap();
    assert!(out.contains("replaced 2 occurrences"), "{out}");
    let t = std::fs::read_to_string(dir.path().join("f.rs")).unwrap();
    assert!(t.contains("    do_other();") && t.contains("        do_other();"), "{t}");
}
...
참고: 위 `do_it();` 케이스는 1단계 정확 일치(부분문자열)가 이미 2곳을 잡으므로 실제로는 1단계 replace_all로 처리된다 — 단언은 결과 텍스트만 검사하므로 유효. 3단계 고유 검증이 필요하면 search를 들여쓰기 없는 여러 줄 블록으로 구성.
...
```rust
        let msg = if outcome.occurrences > 1 {
...
/// 윈도 크기 8: 주기 ≤2 사이클의 5회 정지까지 도달 가능. 주기 3은 교정만 발화하고
/// max_turns가 상한 (더 넓히면 "다른 편집 사이 동일한 실패 테스트"를 오정지)
...
pub struct RepetitionTracker {
    ...
    last_error_first_line: Option<String>,
    error_streak: usize,
}
...
impl RepetitionTracker {
    ...
    pub fn error_correction(&mut self, tool: &str, body: &str) -> Option<&'static str> {
        if !body.starts_with("Error:") {
            self.last_error_first_line = None;
            self.error_streak = 0;
            return None;
        }
        let first = body.lines().next().unwrap_or("").to_string();
        if self.last_error_first_line.as_deref() == Some(first.as_str()) {
            self.error_streak += 1;
        } else {
            self.last_error_first_line = Some(first);
            self.error_streak = 1;
        }
...
async fn four_reads_then_edit_then_reread_is_not_stopped() {
    ...
    let script = Scripted::new(vec![read(), read(), read(), read(), write, read(), ok(&finish("done"))]);
    ...
    let mut session = new_session_guided(&agent);
...
헬퍼 추가 (`new_session`과 나란히):

```rust
    fn new_session_guided(agent: &Agent<&Scripted>) -> Session {
            ...
            if turn.action.tool == "finish" {
                match turn.action.args.get("summary").and_then(|v| v.as_str()) {
                    ...
                    None => {
                        ...
                        session.push(tool_result_message("finish", FINISH_ERR));
...
Run: `cargo test && cargo clippy --all-targets -- -D warnings`
Expected: 전체 PASS — 기존 `finish_returns_summary...` 등은 편집 없는 시나리오라 무변경 통과
...
- [ ] **Step 4: `docs/baselines.md`에 "M5 최종 결과" 절 작성** — 기준선 문서와 동일 형식(전체 통과율 표, 과제별 표, outcome 분포, 관찰), 성공 기준 3항 판정 명기, 잔여 한계 승계(.cargo 홈 벡터 — 스펙 §4.1)
...
- **주의 항목**: Task 4의 `merge` 클로저는 차용 검사에 따라 자유 함수 전환 대안 명기. Task 14는 Task 4가 만든 note 조립부를 대체(선행 의존 명기). grep 폴백(Task 6)은 기존 truncated 경로 보존을 단언으로 강제
