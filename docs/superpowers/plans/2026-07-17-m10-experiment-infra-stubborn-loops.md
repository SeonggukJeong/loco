...
COLS = ["run", "outcome", "passed"] + list(MARKS) + [
    "sr_recovered", "sr_recovery_denom", "perturb_turns", "stop_cause",
]
...
def run_metrics(events):
    ...
    for e in events:
        ...
        if MARKS["sr_error"] in content:
            denom += 1
            pending.append(2)
    return counts, recovered, denom, perturb_turns, last_result
...
def process(stamp_dir):
    ...
    for path in sorted(glob.glob(os.path.join(stamp_dir, "run-*.jsonl"))):
        ...
        counts, rec, den, perturb, last = run_metrics(events)
        ...
        row += [str(rec), str(den), str(perturb), stop_cause(outcome, last)]
        print("\t".join(row))
...
def selftest():
    ...
    counts, rec, den, perturb, _ = run_metrics(events)
    ...
    _, _, _, perturb2, _ = run_metrics(events2)
...
Expected: `run-fix-monthly-total-0` 행에 sr_error=6·stop_cause=sr, `run-update-vat-rate-1` 행에 sr_error=0·stop_cause=other
...
2. 배치 전 게이트: 해당 암 브랜치 체크아웃(`git checkout <branch>`) →
   ...
   `cargo run -- eval tasks-large --verify`(3/3) → `lms unload --all` →
...
- [ ] **Step 2: SrBlock 단위 테스트 작성** — `src/agent/sr_block.rs` 신규 파일 하단에:
...
`src/agent/mod.rs`의 기존 모듈 선언부(파일 상단 `mod repetition;` 근처)에 `mod sr_block;` 추가.
...
async fn two_sr_errors_do_not_block_and_other_file_stays_editable() {
    ...
    let script = Scripted::new(vec![ok(&sr), ok(&read), ok(&sr), ok(&fix_f), ok(&fix_g), ok(&finish("d"))]);
    let mut agent = make_guided_agent(&script, dir.path().to_path_buf(), 25);
    let mut session = new_session(&agent);
    let outcome = run_quiet(&mut agent, &mut session, "x").await.unwrap();
    assert!(matches!(outcome, AgentOutcome::Finished(_)), "{outcome:?}");
...
주의: 첫 테스트에서 read 뒤 finish 요청(요청3)의 온도가 원복인 이유 — read_file 성공이 스트릭을 리셋. `Config::default().temperature`가 0.1임은 config.rs:26 기존 테스트가 보증.
...
공통 config: max_output_tokens 4096, command_timeout_secs 240 (M8 공통 조건).
...
- [ ] **Step 3: 승자 검증 배치** — 8K 결과에서 판정 규칙을 기계 적용해 승자 후보를 정하고(전패면 Task 10으로 — 병합 없음), 러너로 승자 암 @32K + tasks/ 스포트 배치 수행. 스포트는 v2 조건(command_timeout_secs 60·로드 8192 — `.loco/config.toml`을 v2 값으로 임시 조정 후 원복, report.md에 config 기록)
...
- [ ] **Step 3: 문서 갱신** — ① baselines.md: "M10 실험 1" 절(3암 표·행동 지표·판정·스탬프) ② README: 상태 절을 M10으로 ③ CLAUDE.md: M1-M9 라인을 M10 포함으로, --filter·scripts/exp_metrics.py·docs/experiments/ 규약·러너 에이전트·승자 개입 한 줄씩(영문) ④ 스펙 §2 성공 기준 5항 대조표를 report.md에 남긴다
