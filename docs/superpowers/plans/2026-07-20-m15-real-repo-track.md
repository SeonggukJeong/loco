...
- **개정 5** — 4R 전건 반영 + **스펙 개정 11**(§3-4-3의 실행 불가 절차 정정). 아래가 그 요약이다
...
for R in ajeetdsouza/zoxide sharkdp/fd BurntSushi/ripgrep casey/just; do
  N=$(basename "$R")
  # 닫힌 이슈 전 이력 (PR 제외 — gh는 PR도 이슈로 세므로 걸러낸다)
...
```bash
# 이슈 번호 → 그것을 언급하며 닫은 커밋 (전 이력. unshallow는 Step 2에서 끝냈다)
for R in zoxide fd ripgrep just; do
  git -C ~/loco-real-repos/$R.git log --all --format='%H%x09%s%x09%b' \
    | grep -iE '(fix|close|resolve)[sd]? +#[0-9]+' > "$SURVEY/$R-fixcommits.tsv"
  echo "$R 이슈 언급 커밋: $(wc -l < "$SURVEY/$R-fixcommits.tsv")"
done
```
...
3. **§3-2 규약 4 — 백포트 테스트가 부모 트리에서 컴파일되는가.** 새 테스트가 `fix_sha`에서만 존재하는 심볼을 쓰면 부모 트리에서 컴파일이 깨진다. 그러면 `check`가 **컴파일 에러로** 실패해 변별성이 거짓으로 성립한다 — **그 과제는 제외한다.** 확인:

```bash
# 조달(Step 2) 후 사본에서 컴파일만 시켜 본다
CT=$(mktemp -d)/compile && cp -R "$TD/fixture" "$CT" && cd "$CT"
cargo test --no-run 2>&1 | tail -20
```
Expected: **컴파일은 성공하고 테스트만 실패**할 수 있는 상태. `error[E...]`가 나오면 규약 4 위반 → 제외.
...
⚠ **캡처 파일이 비어 있거나 `.rs` 경로가 0건이면 감사를 수행한 것이 아니다** — 그 상태의 rc=0은 "지목되지 않음"이 아니라 "측정 실패"다. 반드시 확인:

```bash
grep -c '\.rs' docs/experiments/2026-07-20-m15-real-repo-baseline/audit/<task>-check.txt
```
Expected: **1 이상**. 0이면 `check` 명령이 출력을 안 냈거나 컴파일 단계에서 죽은 것이다.
...
**1. 4R 지적 반영 — Critical 4 · Important 8 · Minor 13 전건:**

| 축 | 지적 | 반영 |
|---|---|---|
| 통독 C1 | 누설 감사가 입력을 만들 수 없다 → 전 과제 무조건 "지목되지 않음" | **스펙 개정 11**(`b245d73`) + T21 Step 3을 픽스처 사본 실행으로. **`.rs` 0건 = 측정 실패** 확인 단계 추가 |
| 통독 C2 | T21에 `solution/`·`fixture-overlay/` 생성 단계 부재 → A1a가 N/N 실패 | **Step 1b 신설** — diff를 테스트/비테스트로 가르는 명령 + 규약 4 컴파일 확인 |
| 통독 C3 | T20 Step 3(공급량 실사)에 실행 내용 0줄 | **Step 3a~3d 신설** — `gh`+`git log` 이슈↔커밋 연결, 규약 1·3 기계 적용 |
| 실현 축1 C1 | 캐시 히트 대조가 심링크를 안 봄(`ln -s /etc/passwd`가 통과) | `symlinks.txt`·`extracted` 동시 대조 + **"이 검사가 못 보는 것"** 주석 |
| 통독 I1 | T21 입도 — 16과제를 한 커밋에 | Step 8을 **레포 단위 커밋**으로 |
| 통독 I2 | `fixture-overlay`는 `overlay_tree`를 안 탄다(census 오기) | 3개 사이트 전부 "둘"로 정정 |
| 실현 I4 | T17 Step 5 export-ignore 검사가 Expected 도달 불가 | `parent_sha` 갱신 + 재호출, `\|\| true` 제거 |
| 실현 I5 | T19 Step 3이 스펙 §8 각주 6과 정면 모순 | `n_ctx_slot`은 **동결 로드**를 증언하고 실효 운용점의 증인은 `RunRecord`임을 명시 |
| 실현 (비용) | 매니페스트가 파일당 4포크 (N=2000에서 280배 느림) | `manifest_of()` 헬퍼로 `xargs` 일괄 |
| 통독 M1 | 머리말의 순서 주장이 수번과 다름 | "실질 제약의 확인이지 수행 순서가 아니다"로 명시 |
| 실현 M | T19 Step 5의 `grep -c "M15"`가 공허 | `model_diff_trunc`·`M1-M15 done`으로 교체 |

**2. 라운드별 Critical 추이와 그 형태**

| 라운드 | Critical | 축 A | 지적이 나온 곳 |
|---|---|---|---|
| 1R | 6 | — | 코드 태스크 전반 |
| 2R | 3 | **0** | 개정 2가 새로 만든 것 |
| 3R | 2 | **0** | 개정 3이 새로 만든 것 |
| 4R | 4 | **0** | **3건이 "네 개정 내내 손 안 댄 절차 태스크"** |

**네 라운드 연속 축 A가 0이다** — 각 개정의 수정 자체는 매번 실행으로 확인됐다. 4R이 드러낸 것은 **리뷰 설계의 결함**이다: 1R의 3축을 2R·3R에서 계속 좁혔는데 **좁힌 축이 전부 코드 블록을 향했고**, 그 결과 코드 태스크(T2~T19)는 반증 2×2·소비자 전수·`grep -c` 규율을 갖춘 반면 **절차 태스크(T20·T21)는 정련을 한 번도 안 받았다.** 축을 나누는 것은 유효했지만 **축이 서로 다른 *관점*이어야지 서로 다른 *깊이*여선 안 됐다.**

**3. 이 플랜이 실패한 세 형태 (누적)**

1. **검증 장치가 실패할 수 없다** (1R) — 공허한 반증·빈 테스트·자기가 겨눈 단절을 못 잡는 자증
2. **고치는 자리에서 같은 결함을 다시 만든다** (2R·3R) — 나누거나 더하면서 전수를 안 셈
3. **검사를 추가하면서 그 검사가 못 보는 것을 안 센다** (4R) — 매니페스트가 심링크를 안 봄

각각에 규율이 붙어 있다(2×2 채우기 / `grep -c` 후 커밋 메시지에 개수 / **"이 검사가 못 보는 것" 주석**).

**4. 개정 5가 새로 만들 수 있는 것 (5R이 볼 곳)**

- **Step 3a~3d의 셸 파이프라인이 실제로 도는지 미검증이다** — `gh issue list`의 JSON 형식, `git log --format='%H%x09%s%x09%b'`의 다중행 body가 `grep`을 어떻게 통과하는지, 규약 3 판정의 중첩 `while`이 서브셸에서 변수를 잃는지
- **Step 1b의 `git show "$FIX:$f"`가 삭제된 파일에서 실패**한다 — 커밋이 파일을 지웠으면 `solution/`이 그것을 표현할 수 없다(§3-3이 *"삭제·이름변경 표현 불가"*를 이미 적었으나 **명령이 그 경우 죽는다**)
- **`manifest_of()`의 `/tmp/m15-*.$$` 임시 파일**이 동시 실행에서 충돌하지 않는지, `xargs -a`가 BSD에 있는지
- **레포 단위 커밋으로 바꾸면서 Step 번호 참조**(Step 1b·2·3이 "레포마다 반복")가 다른 Step의 서술과 어긋나지 않는지
