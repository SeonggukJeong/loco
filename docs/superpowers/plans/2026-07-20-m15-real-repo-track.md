...
**기준 커밋:** `b245d73` (스펙 **개정 11** — 플랜 1R이 찾은 사실 오류 2건 + 4R이 찾은 실행 불가 절차 1건 정정 반영)
...
- **개정 4** `0d47dbc` — 3R 전건 반영. 4R에서 **Critical 4 · Important 8 · Minor 13**, 2축 모두 `Ready: No`.
  ⚠ **4R의 축 A도 0건** — `row[col[…]]` 전수 치환, 11칸 순서 정합, 단언 ④의 ci95가 변조 (b)를
  정확히 포착, T16 전 항목 일치가 실행으로 확인됐다. **네 라운드 연속 축 A = 0**.
  ⚠⚠ **그런데 4R의 Critical 3건이 "네 개정 내내 손 안 댄 영역"(T20·T21의 절차)에서 나왔다** —
  축을 좁힌 리뷰가 코드 블록만 반복해 훑는 동안 절차 태스크가 사각에 남았다.
  그래서 4R은 **통독 축**(처음 보는 눈으로 전체 읽기)을 새로 붙여 그것을 찾았다
- **개정 5** `7b8e4e8` — 4R 전건 반영 + 스펙 개정 11. 5R(절차 축 단독)에서 **Critical 4 · Important 6 · Minor 5**, `Ready: No`.
  ⚠ 5R은 **실제 4레포에 `gh`/`git`을 돌린 첫 라운드**다. C1은 후보 11 vs 37 · 편중 91% vs 57%로
  **마일스톤의 go/no-go를 뒤집었고**, C2는 squash-merge 레포(just·zoxide)가 구조적으로 0을 내는
  설계 문제였다
- **개정 6** `4154f65` — 5R 전건 반영 + 커밋 전 자기검증에서 2건 추가 발견·해소
  (heredoc/파이프 stdin 충돌, `Fixes`·`Fixed` 정규식 누락). **T19 소비자에
  `PROTOCOL.md:24`(데몬화 레시피) 추가**

## 리뷰 종결 (2026-07-21) — 사용자 판단

**5라운드 · 11축으로 종결한다.** Critical 누적 19건 전건 반영, 스펙도 2회 개정
(개정 10 사실 오류 2건 · 개정 11 실행 불가 절차 1건).

**종결 근거**: 플랜의 **모든 부분이 최소 한 번 실행 검증을 받았다** — 코드 태스크는
1R~4R 4라운드(축 A 4연속 0건), 절차 태스크는 5R 1라운드. 남은 미검증은
`scripts/procure_real.sh`(T17)·`scripts/leak_audit.py`(T18)·`scripts/link_issues.py`(T20)의
**실제 조달·판정 경로**인데, **그 파일들이 아직 없어서** 리뷰가 채울 수 없다.
**만들면서 테스트하는 것이 리뷰를 한 라운드 더 도는 것보다 낫다**(사용자 판단) —
그것이 곧 T17·T18·T20의 구현 단계다.

⚠ **따라서 T17·T18·T20 구현자는 그 태스크의 검증 단계를 특히 성실히 돌 것.**
이 셋은 리뷰가 아니라 **구현이 첫 실행 검증**이 된다.
...
> **개정 3의 규율: 무언가를 "둘로 나누거나" "하나 더 추가"할 때는, 그 이름·목록·분기의 전수를 `grep`으로 다시 세고 개수를 커밋 메시지에 적는다.**

**형태 3 (4R): 검사를 추가하면서 그 검사가 못 보는 것을 안 센다.**

개정 4가 캐시 히트 매니페스트 대조를 추가했는데 **매니페스트는 일반 파일만 담는다.** 그래서 심링크 재지정·삭제·추가가 전부 통과했다 — 실측으로 `ln -s /etc/passwd tree/pwn.txt`가 "손상 또는 변조" 검사를 지나 픽스처로 들어갔다. 같은 라운드에서 T17 Step 5의 export-ignore 검사 블록과 T19 Step 5의 `grep -c "M15"`도 **실패할 수 없는 검증**으로 확인됐다.

> **개정 5가 추가하는 규율: 새 검사를 넣을 때는 그 옆에 "⚠ 이 검사가 못 보는 것"을 주석으로 열거한다.** 못 보는 것이 판정에 도달하지 않는 이유까지 적는다 — 적을 수 없으면 그것은 구멍이다.
...
⚠ **태스크 수번 순서와 스펙의 화살표는 다르다**(4R 통독 M1). `subagent-driven-development`는 T1…T25 수번대로 돌고 T20은 T19 다음이다. 스펙 순서가 요구하는 **실질 제약**은 둘뿐이고 수번 순서가 그것을 만족한다: ① T1(동결)이 모든 데이터 이전 ② T20의 rope 캡처가 T22의 분기 3 판정에 선행. 나머지 화살표(조달→스모크→사전등록→배치)도 T21<T22<T23<T24로 만족된다. 아래 대응은 **실질 제약의 확인**이지 수행 순서가 아니다: T1(동결) → T20(rope+실사) → T2·T3·T8·T17(조달 선행 요건) → T10·T11·T16(축 C·세션 모드) → T21(조달) → T22(스모크·분기) → T23(사전등록) → T24(배치).
...
2. `overlay_tree`(`:107-109`) — 소비자 **둘**: `solution/` 오버레이(`verify.rs:112`)·`sync_protected`의 **디렉터리** 분기(`:54`). ⚠ 개정 2~4는 여기에 `fixture-overlay/`를 세어 **셋**이라 적었는데 **오기다**(4R 통독 I2) — 그것은 T17의 셸 스크립트가 `tar`로 얹으므로 Rust `overlay_tree`를 안 탄다
...
**Consumers:** 심링크 `bail!`의 소비자 — ① `copy_tree`(`Sandbox::create`) ② `overlay_tree`의 `solution/` 오버레이(`verify.rs:112`) ③ `overlay_tree`의 `sync_protected` 디렉터리 분기(`sandbox.rs:54`). **`copy_tree`와 `overlay_tree` 두 함수를 함께 고쳐야 한다.** ⚠ 개정 2~4는 여기에 `fixture-overlay/`를 `overlay_tree` 소비자로 세었는데 **오기다**(4R 통독 I2) — T17의 셸 스크립트가 `tar`로 얹으므로 Rust 경로를 안 탄다. 과대계상이라 수정 자체는 옳았지만, **이것은 이 플랜이 자기 규율의 중심으로 세운 바로 그 형태의 census 오류다.** 그리고 기존 테스트 `symlink_in_fixture_is_an_error`가 이 동작을 고정하고 있으므로 **그 테스트 자체가 갱신 대상 소비자다.**
...
manifest_of() {
  ( cd "$1" && find . -type f | sed 's|^\./||' | LC_ALL=C sort > /tmp/m15-files.$$
    ...
    xargs -a /tmp/m15-files.$$ stat -f '%z' 2>/dev/null > /tmp/m15-sizes.$$ \
      || xargs -a /tmp/m15-files.$$ stat -c '%s' > /tmp/m15-sizes.$$
    xargs -a /tmp/m15-files.$$ $SHA_CMD | awk '{print $1}' > /tmp/m15-hashes.$$
...
⚠ **개정 4의 이 블록은 Expected를 낼 수 없었다**(4R 실현 I4): `.gitattributes`를 커밋·푸시만 하고 **`procure.toml`을 갱신하지도, 스크립트를 재호출하지도 않았다.** 그대로 돌리면 아무것도 안 찍고 exit 0이다. 재호출을 넣어도 `parent_sha`가 그대로면 `fill_cache`가 **히트 경로로 빠져 가드 앞에서 반환**한다. **가드 자체는 건전하다** — `parent_sha`를 `.gitattributes` 커밋으로 맞추면 정상 발동한다(4R 실측).

```bash
cd "$SCRATCH/src"
printf 'c.txt export-ignore\n' > .gitattributes
git add -A && git commit -qm export-ignore-guard
git push -q "$SCRATCH/repos/demo.git" HEAD:master 2>/dev/null \
  || git -C "$SCRATCH/repos/demo.git" fetch -q "$SCRATCH/src" HEAD:refs/heads/master \
  || { echo "bare 갱신 실패 — 검사를 수행하지 못했다"; exit 1; }
# ⚠ **parent_sha를 .gitattributes 커밋 자신으로** 맞춘다. 안 그러면 캐시 히트로
#    빠져 가드에 도달조차 못 한다
GUARD_SHA=$(git rev-parse HEAD)
sed -i '' -e "s/^parent_sha = .*/parent_sha = \"$GUARD_SHA\"/" "$SCRATCH/task/procure.toml"
cd /Users/sgj/develop/loco
LOCO_REAL_REPOS="$SCRATCH/repos" LOCO_TASKS_REAL_CACHE="$SCRATCH/cache" \
  /bin/sh scripts/procure_real.sh "$SCRATCH/task"; echo "EXIT=$?"
```
Expected: `export-ignore 의심 — ls-tree에 있고 아카이브에 없는 경로:` + `c.txt` + `EXIT=1`, 그리고 trap이 부분 캐시 디렉터리를 지운다.

⚠ **`EXIT=0`이면 가드가 공허한 것이다.** 특히 `캐시 히트` 줄이 보이면 `parent_sha` 갱신이 안 먹은 것이니 그것부터 고칠 것.
⚠ 위 `|| true`를 쓰지 말 것 — 개정 4는 세 명령의 실패를 전부 삼켜 **가드에 도달하지 못한 것과 가드가 안 문 것을 구별할 수 없었다.**
...
- **`PROTOCOL.md:24` 4④(데몬화 레시피)** — ⚠ 현재 레시피는 **대화형 셸에서 `PermissionError`로 즉사한다**(5R Critical 4). 배경 프로세스가 이미 프로세스 그룹 리더라 `setsid()`가 EPERM이다. **fork 후 setsid** 형태로 고칠 것:
  ```
  python3 -c "
  import os,sys
  ...
  " <cmd>...
  ```
  적용 시점을 **M15 이후**로 표시한다 — M13·M14 배치가 이 레시피로 돌았다면 그것은 비대화형 경로였다는 뜻이므로 소급 재해석하지 않는다
...
```markdown
   ...
   "어떤 컨텍스트로 돌았는가"에 답하지 못한다 (M15 §6-4-17) ...
```
...
grep -c "model_diff_trunc" CLAUDE.md      # Step 4b 미수행이면 0
grep -c "M1-M15 done" CLAUDE.md          # Step 4 미수행이면 0 (기존은 "M1-M14 done")
# ⚠ `grep -c "M15" CLAUDE.md`를 쓰지 말 것 — 커맨드 3줄만으로 이미 nonzero라
#    **Step 4·4b를 통째로 건너뛰어도 통과한다**(4R 실현 M). 실패할 수 있는 검사여야 한다
```
...
⚠ **개정 4까지 이 자리는 주석 세 줄이었고 명령이 0줄이었다**(4R 통독 C3). 이 단계가 정하는 것이 **N·레포 선정·§6-4-2 미달 처분 발동 여부·A1b**인데도 그랬다. 아래가 실행 가능한 절차다.

- [ ] **Step 3a: 닫힌 이슈 ↔ 수정 커밋을 잇는다**

`gh`는 이슈 목록을, `git log`는 그 이슈를 닫은 커밋을 준다. GitHub는 `Fixes #N`/`Closes #N` 본문 규약으로 잇는다:

```bash
SURVEY=docs/experiments/2026-07-20-m15-real-repo-baseline/survey
mkdir -p "$SURVEY"
for R in ajeetdsouza/zoxide sharkdp/fd BurntSushi/ripgrep casey/just; do
  N=$(basename "$R")
  # 닫힌 이슈 전 이력. ⚠ `gh issue list`는 **이미 PR을 제외한다**(5R Minor 3: Search API
  #    대조로 확인 — zoxide 648/643, fd 847/847, ripgrep 1696/1695, just 1283/1283).
  #    개정 5의 "gh는 PR도 이슈로 센다"는 주석은 사실과 달랐고 거르는 코드도 없었다
  gh issue list -R "$R" --state closed --limit 2000 \
     --json number,title,url,closedAt > "$SURVEY/$N-issues.json"
  echo "$N 닫힌 이슈: $(python3 -c "import json,sys;print(len(json.load(open('$SURVEY/$N-issues.json'))))")"
done
```

⚠ **레이트리밋**: `gh`는 인증 시 5000 req/h다. 위는 레포당 요청 수 개라 여유롭다. `gh auth status`로 먼저 확인할 것.

⚠ **`--limit`은 상한 도달 시 에러 없이 조용히 절단된다**(5R Important 6: ripgrep 1696/2000). §6-4-1이 요구하는 것은 **전 이력**이므로 절단을 검출한다:

```bash
for N in zoxide fd ripgrep just; do
  C=$(python3 -c "import json;print(len(json.load(open('$SURVEY/$N-issues.json'))))")
  [ "$C" -ge 2000 ] && echo "⚠ $N: $C — --limit에 닿았다. 상한을 올려 재수집할 것" >&2
done
```
실측 참고(5R): zoxide 648 / fd 847 / ripgrep 1696 / just 1283 — 현재는 2000이 충분하나 시간이 지나면 닿는다.
...
- [ ] **Step 3a-2: squash-merge 레포는 다른 방법이 필요하다**

⚠⚠ **just·zoxide는 이 방식으로 구조적으로 0을 낸다**(5R Critical 2 실측: just 0/2283, zoxide 2/627 — 반면 `#N` 언급 자체는 just 1777, zoxide 252건 있다). 두 레포는 **squash-merge + `(#PR)` 관례**라 이슈가 **PR 본문으로 닫히고 커밋 메시지에 흔적이 없다**(리뷰어가 `issues/N/timeline`에서 `commit_id: null` 확인). **그런데 그 0이 레포 선정과 §6-4-3 편중 상한 계산에 그대로 들어간다.**

```bash
# 커밋 연결이 희박한 레포는 GitHub timeline API로 잇는다.
# closed_by → PR → merge commit 경로. gh api는 페이지네이션을 --paginate로 처리한다
for R in ajeetdsouza/zoxide casey/just; do
  N=$(basename "$R")
  python3 -c "import json,sys;[print(i['number']) for i in json.load(open('$SURVEY/$N-issues.json'))]" \
  | while IFS= read -r num; do
      gh api "repos/$R/issues/$num/timeline" --paginate \
        --jq '.[] | select(.event=="cross-referenced" or .event=="closed") 
              | (.commit_id // .source.issue.pull_request.url // empty)' 2>/dev/null \
        | head -1 | sed "s|^|$num\t|"
    done > "$SURVEY/$N-timeline.tsv"
  echo "$N timeline 연결: $(wc -l < "$SURVEY/$N-timeline.tsv")"
done
```

⚠ **timeline API는 이슈당 1요청이다** — zoxide 648 + just 1283 = **1,931 요청**. `gh`의 인증 한도는 5000 req/h이므로 한 번에 들어가지만, **`gh auth status`로 먼저 확인하고 429가 나면 중단한 지점을 기록**할 것.

⚠ **연결 방식이 레포마다 다르면 그 사실을 사전등록에 적는다** — 공급량 수치의 비교가능성에 영향을 준다(레포 A는 커밋 메시지, 레포 B는 timeline API).

- [ ] **Step 3b: 테스트를 동반한 수정만 남긴다** (§3-2 채택 조건)

```bash
# 각 fix 커밋이 테스트 파일을 추가·수정했는가
for R in zoxide fd ripgrep just; do
  : > "$SURVEY/$R-with-tests.tsv"
  cut -f1 "$SURVEY/$R-fixcommits.tsv" | while IFS= read -r sha; do
    files=$(git -C ~/loco-real-repos/$R.git show --name-only --format= "$sha")
    # 테스트 = tests/ 아래이거나 파일명에 test가 든 .rs
    if printf '%s\n' "$files" | grep -qE '(^|/)tests?/|_test\.rs$|test_.*\.rs$'; then
      printf '%s\t%s\n' "$sha" "$(printf '%s\n' "$files" | tr '\n' ',')" \
        >> "$SURVEY/$R-with-tests.tsv"
    fi
  done
  echo "$R 테스트 동반: $(wc -l < "$SURVEY/$R-with-tests.tsv")"
done
```

- [ ] **Step 3c: 제외 규약 1~5를 기계로 적용한다**

⚠ **규약 6(§3-4-3 지목됨)은 여기서 못 적용한다** — 조달과 `check` 실행이 필요하다. T21에서 걸러지므로 **이 단계의 최종 수는 상한이지 확정이 아니다.**

```bash
# 규약 1: 여러 이슈를 함께 고친 커밋 → 제외 (본문의 #N 언급이 2개 이상)
# 규약 3: 테스트 변경이 **수정 대상과 같은 파일** → 제외
#         (protected는 경로 단위이고 헝크 단위 보호 기구가 없다)
# 규약 5: .cargo/를 건드림 → 규약 2와 같은 형태(그 부분 제외 후 --verify 통과 시 채택)
for R in zoxide fd ripgrep just; do
  : > "$SURVEY/$R-candidates.tsv"
  while IFS=$'\t' read -r sha files; do
    n_issues=$(git -C ~/loco-real-repos/$R.git show -s --format='%s %b' "$sha" \
               | grep -oiE '#[0-9]+' | sort -u | wc -l | tr -d ' ')
    [ "$n_issues" -gt 1 ] && continue                       # 규약 1
    # 규약 3 — 한 파일이 테스트이면서 동시에 비테스트 소스 변경분에 속하는가
    same_file=$(printf '%s' "$files" | tr ',' '\n' \
      | grep -E '\.rs$' | while IFS= read -r f; do
          case "$f" in */tests/*|tests/*) continue;; esac
          # cfg(test)를 품은 소스 파일이 수정됐으면 헝크 단위 충돌 위험
          git -C ~/loco-real-repos/$R.git show "$sha:$f" 2>/dev/null \
            | grep -q '#\[cfg(test)\]' && echo hit
        done)
    [ -n "$same_file" ] && continue                          # 규약 3
    printf '%s\t%s\n' "$sha" "$files" >> "$SURVEY/$R-candidates.tsv"
  done < "$SURVEY/$R-with-tests.tsv"
  echo "$R 규약 1·3 통과: $(wc -l < "$SURVEY/$R-candidates.tsv")"
done
```

⚠⚠ **규약 3의 조작적 정의는 proxy로 확정한다** — 그리고 그 근거는 **수치가 아니라 스펙의 *이유*다.**

스펙 §3-2 규약 3의 문언은 *"테스트 변경이 수정 대상과 같은 파일 — 제외"*이고, 붙은 이유가 *"`protected`는 경로 단위이고 헝크 단위 보호 기구가 없다"*이다. 그 이유를 그대로 따르면 **수정 대상 파일이 `#[cfg(test)]`를 품기만 하면** — 이번 커밋이 그 mod를 안 건드렸더라도 — 그 파일을 protected로 걸 수 없고, 걸지 않으면 판정 자산이 무방비다. 따라서 **"커밋이 건드린 비테스트 `.rs`에 `#[cfg(test)]`가 있는가"가 그 이유의 올바른 조작적 정의**이며, 문언을 좁게 읽어 "이번 커밋이 테스트 mod도 함께 고쳤는가"로 한정하면 이유가 보호하려던 위험이 그대로 남는다.

⚠ **무결성 기록 — 순서를 명시한다.** 5R 리뷰가 두 정의의 수치를 **먼저 드러냈다**: proxy면 후보 37건·최대 편중 57%(§6-4-3 상한 60% 통과), 문언대로면 61건·72%(초과 → §6-4-2 처분 발동). **이 선택은 그 수치를 본 뒤에 내려졌다.** 그래서:
- **임계값 자체(최소 표본 16·편중 상한 60%)는 T1에서 어떤 데이터도 보기 전에 동결됐다**(`thresholds.md`, 커밋 해시 기록). 조정한 것은 임계값이 아니라 **측정 방법의 조작적 정의**다
- **사전등록에 두 정의의 수치를 나란히 적는다** — proxy(주 분석)와 문언(민감도). 한쪽만 적으면 선택의 자유도가 감춰진다
- 5R의 수치는 **리뷰어의 드라이런**이고 §6-4-1의 공식 실사는 T20이 정정된 스크립트로 다시 돌려 기록한다

⚠ **규약 2·4·5는 사람이 본다** — 의존성 업데이트·포매팅 동반(2), 백포트 테스트가 부모 트리에서 컴파일되는가(4), `.cargo/` 접촉(5). 규약 4는 실제로 컴파일해 봐야 알 수 있어 **T21에서 확정된다.**

⚠ 실측 참고(3R): `cfg(test)` 포함 파일이 ripgrep `crates/` **44개**, fd `src/` **13개** — 규약 3이 후보를 크게 줄인다.
...
- [ ] **Step 1b: `fixture-overlay/`와 `solution/`을 만든다 — 이 태스크에서 판단이 가장 많이 드는 작업**

⚠ **개정 4까지 이 단계가 아예 없었다**(4R 통독 C2). 두 산출물이 `Files` 헤더와 §3-3 표에만 있고 9개 Step 어디에도 없어서, `verify.rs:82-84`가 `solution/` 부재로 즉사하며 **A1a가 첫 실행에서 N/N 실패**할 상태였다.

§3-3이 못박은 구성(성립하는 유일한 형태)을 **수정 커밋 diff를 둘로 갈라서** 만든다:

| 산출물 | 내용 | 근거 |
|---|---|---|
| `fixture-overlay/` | 커밋의 **테스트 변경분만** — 조달이 부모 트리 위에 얹는다 | `verify.rs:117`이 `(!step1, step2)`라 **실패하는 테스트가 픽스처 안에 있어야** 변별성이 성립 |
| `solution/` | 커밋 diff **− 테스트 변경분** (= 비테스트 소스만) | `verify.rs:86-88`이 `solution/`과 protected의 겹침을 거부 |

```bash
TD=tasks-real/<task>; FIX=<fix_sha>; REPO=<repo>
G=~/loco-real-repos/$REPO.git
mkdir -p "$TD/fixture-overlay" "$TD/solution"

# 커밋이 건드린 파일을 테스트/비테스트로 가른다
git -C "$G" show --name-only --format= "$FIX" | grep -v '^$' > /tmp/m15-files.txt
grep -E '(^|/)tests?/|_test\.rs$|test_.*\.rs$' /tmp/m15-files.txt > /tmp/m15-test.txt || true
grep -vE '(^|/)tests?/|_test\.rs$|test_.*\.rs$' /tmp/m15-files.txt > /tmp/m15-src.txt || true

# fixture-overlay/ = 테스트 파일의 **수정 후**(fix_sha) 내용
while IFS= read -r f; do
  mkdir -p "$TD/fixture-overlay/$(dirname "$f")"
  git -C "$G" show "$FIX:$f" > "$TD/fixture-overlay/$f"
done < /tmp/m15-test.txt

# solution/ = 비테스트 소스의 **수정 후** 내용 (파일 단위 오버레이 — 삭제·이름변경 표현 불가)
while IFS= read -r f; do
  mkdir -p "$TD/solution/$(dirname "$f")"
  git -C "$G" show "$FIX:$f" > "$TD/solution/$f"
done < /tmp/m15-src.txt

# ⚠ CHANGELOG·문서를 solution/에서 뺀다 (§5-4 제약 2의 오라클 오염과 같은 사정 —
#    레포마다 관례가 달라 자동 규칙이 아니라 **사람이 본다**)
rm -f "$TD/solution/CHANGELOG.md" "$TD/solution/README.md"
```

**그리고 세 가지를 손으로 확인한다:**

1. **`protected`를 `fixture-overlay/`의 테스트 경로로 맞춘다** — `task.toml`의 `protected`가 `/tmp/m15-test.txt`의 경로와 일치해야 한다(`task.rs:67-71`이 픽스처에 없는 protected를 `bail!`한다)
2. **`check`가 그 테스트를 지목하는지** — 필터가 아무것도 못 잡으면 exit 0 → 픽스처 PASS → **판별력 실패**로 걸린다(M12형 0-테스트 거짓 초록불도 이것이 걷어낸다)
3. **§3-2 규약 4(백포트 테스트가 부모 트리에서 컴파일되는가)는 Step 2b에서 확인한다** — `fixture/`가 아직 없으므로 여기서는 못 한다.

⚠ **개정 5는 이 확인을 여기 뒀는데 `fixture/`는 Step 2(조달)가 만든다.** `cp`가 실패하고 `cd`가 안 되어 **loco 레포 자신을 컴파일하고** `error[E...]`가 없으니 "통과"로 읽혔다(5R Critical 3 실측). 주석에 *"조달(Step 2) 후"*라고 적어 놓고 배치를 틀렸다 — 플랜 서두가 금지한 **형태 1: 검증 장치가 실패할 수 없다** 그 자체다.

4. **`solution/`이 비면 그 과제를 여기서 뺀다**(5R Important 3). 후보의 **22%가 테스트 전용 커밋**이다(ripgrep 7건 — `['CHANGELOG.md','tests/regression.rs']` 형태). `solution/`이 비면 부모 트리에 버그가 이미 없어 **변별성이 깨진다.** Step 4의 `--verify`가 잡지만 그때는 레포 단위 작업을 다 한 뒤다:

```bash
if [ -z "$(find "$TD/solution" -type f -print -quit)" ]; then
  echo "solution/이 비었다 — 테스트 전용 커밋으로 보인다. 이 과제는 제외한다: $TD" >&2
  exit 1
fi
```

5. **삭제·이름변경 커밋은 제외한다**(5R Important 5). `git show "$FIX:$f"`는 삭제된 파일에서 rc=128인데 `>` 리다이렉트가 **0바이트 파일을 남긴다** — `solution/`이 "삭제"가 아니라 "빈 파일 오버레이"가 되어 조용히 오염된다. §3-3이 *"삭제·이름변경 표현 불가"*를 이미 적었으므로 처분은 제외다. 위 두 루프의 `git show`를 이렇게 감쌀 것:

```bash
  if ! git -C "$G" show "$FIX:$f" > "$TD/solution/$f" 2>/dev/null; then
    rm -f "$TD/solution/$f"
    echo "삭제·이름변경 포함 커밋 — 제외: $TD ($f)" >&2; exit 1
  fi
```

⚠ 현재 fd/ripgrep 후보 풀에는 삭제·이름변경 커밋이 **0건**이지만(5R 실측) 가드가 없으면 다음 배치에서 걸린다.

⚠ **여기가 §10-4가 말한 "컨트롤러 저작이 다시 들어오는" 자리다** — 전제는 외부인이 썼지만 diff를 어떻게 가를지는 컨트롤러 판단이다. **완화책이지 제거책이 아니며, 이 편향은 남는다고 리포트에 명시한다.**
...
- [ ] **Step 2b: 규약 4 — 백포트 테스트가 부모 트리에서 컴파일되는가**

조달이 `fixture/`를 만든 **뒤**에만 할 수 있다. 새 테스트가 `fix_sha`에서만 존재하는 심볼을 쓰면 부모 트리에서 컴파일이 깨지고, 그러면 `check`가 **컴파일 에러로** 실패해 **변별성이 거짓으로 성립한다.**

```bash
CT=$(mktemp -d)/compile
cp -R "tasks-real/<task>/fixture" "$CT"
( cd "$CT" && cargo test --no-run ) > /tmp/m15-compile.log 2>&1; rc=$?
tail -20 /tmp/m15-compile.log
echo "rc=$rc"
rm -rf "$CT"
```
Expected: **`rc=0`** — 컴파일은 성공하고 테스트만 실패할 수 있는 상태.

⚠ **`rc`를 봐야 한다.** 개정 5는 `cargo test --no-run 2>&1 | tail -20`로 썼는데 **파이프가 rc 101을 0으로 마스킹한다**(5R Minor 1) — M14가 하네스에서 닫으려는 바로 그 혼동이 검증 절차 안에 있었다. 위처럼 리다이렉트 후 `rc`를 따로 읽는다.

⚠ `rc≠0`이거나 로그에 `error[E`가 있으면 **규약 4 위반 → 그 과제 제외.**
⚠ 서브셸 `( cd … )`을 쓰는 이유: `cd`가 셸 cwd를 임시 디렉터리에 남기면 뒤따르는 상대 경로 명령이 깨진다(5R Minor 2).
...
⚠⚠ **`--verify`로는 이 감사를 할 수 없다**(4R 통독 C1, **스펙 개정 11이 정정**). `--verify`는 `check` 출력을 어디에도 내보내지 않는다 — `verify.rs`의 `run_check`가 `ExecEnd::Done(s) => Ok(s.success())`로 **bool만 돌려주고 `exec.body`를 버리고**, `exec_shell`이 `Stdio::piped()`라 자식 출력이 터미널로 새지도 않는다(`exec.rs:122-123`). 그러면 캡처 파일에 `.rs` 경로가 0건이고 `leak_audit.py`가 **전 과제에 대해 무조건 "지목되지 않음"**을 내 §3-2 규약 6이 **통과만 하는 게이트**가 된다. `sb.root`도 로깅되지 않고 `cleanup()`이 지우므로 `--sandbox` 인자 자체가 획득 불가였다.

**픽스처 사본에서 돌린다** — 원래 지시의 이유(`target/` 가드를 안 깬다)가 그대로 만족되고, 사본 경로가 곧 `--sandbox` 접두다:

```bash
AUD=$(mktemp -d)/audit
cp -R tasks-real/<task>/fixture "$AUD"
CHECK=$(python3 -c "import tomllib,sys;print(tomllib.load(open('tasks-real/<task>/task.toml','rb'))['check'])")
( cd "$AUD" && RUST_BACKTRACE=0 sh -c "$CHECK -- --test-threads=1" ) > \
  docs/experiments/2026-07-20-m15-real-repo-baseline/audit/<task>-check.txt 2>&1
python3 scripts/leak_audit.py \
  docs/experiments/2026-07-20-m15-real-repo-baseline/audit/<task>-check.txt \
  --sandbox "$AUD" --oracle <oracle_files의 각 항목>
rm -rf "$AUD"
```

⚠ **가드는 `protected` 경로와 cargo 배너를 빼고 세야 판별력이 있다**(5R Important 1·4 실측). 단순 `grep -c '\.rs'`는 세 이유로 **언제나 충족된다**: ① 단언 실패가 **항상** `tests/<protected>.rs`를 찍는다 ② cargo가 `Running unittests src/main.rs` 배너를 무조건 찍는다 ③ **0-테스트 rc=0 런에서도** 배너 2줄 때문에 통과한다(M12형 거짓 초록불이 감사 절차 안에서 재현된다).

```bash
CAP=docs/experiments/2026-07-20-m15-real-repo-baseline/audit/<task>-check.txt
# 실패 보고 구간에서, protected 경로와 Running/Compiling 배너를 뺀 .rs 수
grep -vE '^\s*(Running|Compiling|Finished)' "$CAP" \
  | grep -oE '[A-Za-z0-9_./-]+\.rs' | grep -v '<protected 경로>' | sort -u | wc -l
# 테스트가 실제로 돌았는지 (0-테스트 rc=0 배제 — M12 §2-4와 같은 판정)
grep -E '^test result:' "$CAP"
```
Expected: 첫 값 **1 이상**, 그리고 `test result: FAILED. … 1 failed` 형태로 **실제 실패가 1건 이상**. 둘 중 하나라도 아니면 그 rc=0은 "지목되지 않음"이 아니라 **"측정 실패"**다 — 다시 돌린다.

⚠ **`check`는 `--test <name> <filter>` 형식이어야 한다**(5R Important 4). 맨 `cargo test`면 cargo가 `Running unittests src/main.rs`를 찍는데 fd 후보 15건 중 **11건이 오라클 = `src/main.rs`**라 순진한 매칭이면 전건 "지목됨 → 제외"가 된다. 기존 `tasks/`는 전부 맨 `cargo test` 형식이므로 **`tasks-real`에는 템플릿 형식을 강제**한다.
⚠ **`check`에 `--`가 이미 있거나 파이프가 있으면 `-- --test-threads=1` 추가가 깨진다**(5R Minor 5: 각각 0-테스트 rc=0 / `tail: --test-threads=1: No such file`). 그런 `check`는 쓰지 말 것.
...
- [ ] **Step 8: 레포 단위로 커밋한다** (입도 — 4R 통독 I1)

⚠ **한 커밋에 16과제를 통째로 담지 말 것.** 개정 4는 Step 8 하나에서 `git add tasks-real`을 통째로 했는데, 그러면 **리뷰어가 16개 픽스처를 통째로 승인하거나 거절해야 하고**(스킬 기준: *"split only where a reviewer could meaningfully reject one task while approving its neighbor"*), 12번째 과제가 규약 6에 걸렸을 때 남는 기록이 없다.

**Step 1b~7을 레포 단위로 반복하고, 레포마다 게이트+커밋을 둔다:**

```bash
for REPO in <선정된 레포들>; do
  # …그 레포의 과제들에 대해 Step 1·1b·2·3 수행…
  cargo run -- eval tasks-real --verify --filter <그 레포의 과제들> 2>&1 | tail -3
  find tasks-real -maxdepth 3 -name target -print   # 0건
  git add tasks-real/<그 레포의 과제들> docs/experiments/2026-07-20-m15-real-repo-baseline/audit/
  git commit -m "feat(tasks-real): <repo> N과제 조달·3항 감사 (§3-2·§3-4)"
done
```

레포 전부가 끝난 뒤 전역 게이트와 동결:

```bash
cargo test && cargo clippy --all-targets -- -D warnings
cargo run -- eval tasks-real --verify 2>&1 | tail -3   # 전 과제 N/N
git add docs/experiments/2026-07-20-m15-real-repo-baseline/frozen-sample.md
git commit -m "docs(m15): 표본 동결 — N과제, 실격 대역 절대값 (§6-4-4·§6-4-6)"
```

⚠ **레포 단위 커밋 하나가 곧 리뷰 단위다** — 그 단위에서 규약 6 탈락이 나오면 그 커밋만 되돌리면 된다.
...
⚠⚠ **`PROTOCOL.md:24`의 레시피를 그대로 쓰면 죽는다**(5R Critical 4 실측). job control이 켜진 셸 — 즉 **사람이 6~10시간 배치를 띄우는 모든 대화형 셸** — 에서 배경 프로세스는 **이미 프로세스 그룹 리더**라 `setsid()`가 `PermissionError: [Errno 1] Operation not permitted`로 즉사한다. 배치는 시작조차 안 되고 로그만 남는다 — **이 명령이 막으려던 "60분 상한 2회 사망"과 같은 형태의 실패다.**
...
python3 -c "
import os,sys
if os.fork(): os._exit(0)      # 부모 즉시 종료 → 자식은 그룹 리더가 아니다
os.setsid()
...
```bash
ps -o pid,ppid,pgid,sid,command -p $(pgrep -f 'eval tasks-real')
```
Expected: `PID == SID` (자기 세션 리더 = 데몬화 성공). `PermissionError`가 보이면 fork 없이 돌린 것이다.

⚠ **`PROTOCOL.md:24`도 함께 고친다** — T19의 소비자 목록에 이 항목을 더할 것.
...
**1. 5R 지적 반영 — Critical 4 · Important 6 · Minor 5 전건:**

| 지적 | 반영 |
|---|---|
| C1 다중행 body가 `cut -f1` 파괴 (go/no-go 전복) | Step 3a를 **NUL 구분 + 레코드 단위 파이썬 처리**로. SHA 40자 검사로 조용한 오염 차단 |
| C2 squash-merge 레포가 구조적 0 (just·zoxide) | **Step 3a-2 신설** — timeline API 연결. 요청량(1,931)과 한도 명시, 방식 차이를 사전등록에 기록 |
| C3 규약 4 확인이 `fixture/` 생성 이전 → loco 자신을 컴파일 | **Step 2b로 이동**(조달 후). `rc` 직접 확인, 서브셸 `cd` |
| C4 데몬화가 대화형 셸에서 EPERM 즉사 | **fork 후 setsid** + `PID == SID` 확인. **T19 소비자에 `PROTOCOL.md:24` 추가** |
| I1 누설 감사 가드가 언제나 충족 | protected 경로·배너 제외 + `test result:` 실제 실패 확인 |
| I2 규약 3 proxy vs 문언이 편중 게이트 전복 | **proxy 확정** — 근거는 스펙의 *이유*(경로 단위 protected). **무결성 기록 3항** 병기 |
| I3 `solution/` 공집합 (후보의 22%) | Step 1b에 조기 탈락 검사 |
| I4 cargo 배너 오탐 (fd 15중 11건) | `check` 템플릿 형식 강제 |
| I5 삭제 커밋의 0바이트 오염 | `git show` rc 검사 + 제외 처분 |
| I6 `--limit` 조용한 절단 | 도달 검출 루프 |
| M1 `\| tail`이 rc 101→0 마스킹 | 리다이렉트 후 rc 분리 |
| M2 `cd`가 셸 cwd 오염 | 서브셸 |
| M3 "gh는 PR도 센다" 오류 | 주석 정정 (Search API 대조값 기록) |
| M4 규약 1이 PR 참조를 이슈로 셈 | 2열 이슈 목록 개수로 판정 |
| M5 `--` 중복·파이프 `check` | 금지 명시 |

**2. 라운드별 추이 — 그리고 이번 라운드의 의미**

| 라운드 | Critical | 축 A | 지적이 나온 곳 |
|---|---|---|---|
| 1R | 6 | — | 코드 태스크 전반 |
| 2R | 3 | 0 | 개정 2가 만든 것 |
| 3R | 2 | 0 | 개정 3이 만든 것 |
| 4R | 4 | 0 | **3건이 미검증 절차 태스크** |
| 5R | 4 | — | **4건 전부 개정 5가 신설한 절차 코드** |

5R은 **개정 5가 신설한 셸 파이프라인 셋의 첫 실행**이었다. 새 코드가 첫 실행에서 Critical을 내는 것은 예상된 비용이고, **이로써 플랜의 모든 부분이 최소 한 번 실행 검증을 받았다** — 코드 태스크는 4라운드(축 A 연속 0), 절차 태스크는 5R 1라운드.

**3. 이 플랜이 실패한 세 형태 (누적, 규율 포함)**

1. **검증 장치가 실패할 수 없다** (1R·4R·5R) → 규율: **2×2를 채운다**(정상=통과 / 변조=실패)
2. **고치는 자리에서 같은 결함을 다시 만든다** (2R·3R) → 규율: **`grep -c`로 전수를 세고 개수를 커밋 메시지에 적는다**
3. **검사를 추가하면서 그 검사가 못 보는 것을 안 센다** (4R) → 규율: **"⚠ 이 검사가 못 보는 것"을 주석으로 열거한다**

⚠ **형태 1이 세 라운드에 걸쳐 반복됐다**(1R 6건 중 4건, 4R C1, 5R C3·I1). 가장 끈질긴 형태다.
