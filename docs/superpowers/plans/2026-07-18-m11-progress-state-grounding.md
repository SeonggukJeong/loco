...
1. 목적·데이터(스탬프 3종·리워드 픽스처 기준임을 명시)
...
- Produces: 런별 컬럼 `first_mut_turn`/`cargo_after_mut`/`zero_mut_end`/`status_in_args`/`sr_files`, 요약의 zero_mut 분해 — Task 9·10의 판정 입력
...
fn remove_status_note_survives_elided_messages() {
    ...
    s.pack(800); // 첫 tool_result 본문 생략 — 접미(상태선)는 pack이 보존한다
...
async fn repetition_stop_still_fires_with_status_note_active() {
    // 채널 격리 회귀: 상태선이 있어도 동일 호출 5회는 RepetitionStop —
    // note는 body 해시 밖 (기존 five_identical_calls 테스트의 상태선 하 재확인)
    let dir = tempfile::tempdir().unwrap();
...
Expected: FAIL — 상태선이 아직 미배선(0개 매치로 assert 실패; `repetition_stop_still_fires...`는 통과해도 무방)
...
Expected: 신규 5건 포함 전건 PASS. 기존 테스트 중 5턴 이상 무뮤테이션 시나리오(예: finish_nudge·repetition 계열)가 상태선 주입으로 어서션이 흔들리면, **기대값을 상태선 포함으로 갱신하는 것이 맞는지 스펙 §4 발동 조건과 대조 후** 갱신한다(무단 완화 금지 — 흔들린 테스트 목록을 커밋 메시지에 남긴다).
...
Expected: 전건 통과 후 브랜치 삭제 없음(M10 관례 — 실험 이력 보존은 병합 커밋이 담당, 브랜치는 삭제해도 무방하나 패자가 아니므로 삭제).
