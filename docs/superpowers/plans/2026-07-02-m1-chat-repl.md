...
- reqwest는 `default-features = false, features = ["json", "stream", "rustls-tls"]` — OpenSSL 의존 금지
...
cargo add reqwest --no-default-features --features json,stream,rustls-tls
...
- Produces:
  ...
  - `OpenAiClient::chat(&self, req: &ChatRequest) -> Result<ChatResponse, LlmError>` — 연결 실패/5xx에 지수 백오프(200ms, 400ms)로 총 3회 시도
...
mod tests {
    ...
    async fn chat_gives_actionable_error_when_server_down() {
        ...
        assert!(msg.contains("127.0.0.1:9"), "주소가 포함돼야 함: {msg}");
...
Run: `cargo test client`
Expected: 5 passed (재시도 테스트는 백오프 때문에 ~0.2초 걸림)
...
- Produces: `OpenAiClient::chat_stream(&self, req: &ChatRequest, on_delta: &mut dyn FnMut(&str)) -> Result<String, LlmError>` — 델타마다 콜백 호출, 전체 텍스트 누적 반환. 호출 측에서 `req.stream = true`로 설정해야 함.
...
- Produces:
  - `OpenAiClient::list_models(&self) -> Result<Vec<String>, LlmError>` — `GET {base}/models`의 `data[].id`
  - `resolve_model(client: &OpenAiClient, config: &Config) -> anyhow::Result<String>` (자유 함수, `src/llm/client.rs`에 정의) — config.model이 비어있지 않으면 그대로, 비어있으면 서버 첫 모델, 그것도 없으면 "서버에 로드된 모델이 없습니다" 에러
...
- Produces:
  ...
  - `run_repl(client: &OpenAiClient, config: &Config, model: &str) -> anyhow::Result<()>`
...
`./.loco/config.toml` (프로젝트) 또는 전역 config_dir의 `loco/config.toml`:
...
Expected: 각각 스트리밍 응답 / 설정 표시 / 초기화 메시지 / 정상 종료

서버가 꺼진 상태에서:

Run: `cargo run -- -p "hi"` (LM Studio 종료 후)
Expected: "서버에 연결할 수 없습니다 (http://localhost:1234/v1)..." 안내 후 종료
