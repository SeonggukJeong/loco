...
- 각 태스크 완료 시점에 `cargo test` 전체 통과 + `cargo clippy` 경고 없음
...
- Consumes: 없음
- Produces:
  - `Config` 구조체: `base_url: String`, `api_key: Option<String>`, `model: String`, `temperature: f32`, `context_tokens: usize`, `max_turns: usize`, `command_timeout_secs: u64`
...
- Produces:
  - `async fn OpenAiClient::list_models(&self) -> Result<Vec<String>, LlmError>` — `GET {base}/models`의 `data[].id`
...
```rust
    ...
    async fn resolve_model_prefers_config_value() {
        ...
        // 서버가 죽어있어도 config에 모델이 있으면 네트워크를 타지 않는다
...
printf 'base_url = "http://127.0.0.1:59999/v1"\n' > "$SMOKE/.loco/config.toml"
(cd "$SMOKE" && cargo run --manifest-path /Users/sgj/develop/loco/Cargo.toml -- -p "hi"; echo "exit=$?")
```

Expected: "서버에 연결할 수 없습니다 (http://127.0.0.1:59999/v1)..." 안내 출력, `exit=1`
(model 미설정이라 `list_models`가 재시도 없이 빠르게 실패함)
...
- M3 계획: 쓰기/실행 툴 + 확인 게이트 + diff 미리보기 + 히스토리 절삭
