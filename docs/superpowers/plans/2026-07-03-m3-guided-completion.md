...
6. **deny 패턴은 --auto에서만 차단** (스펙 §5). 대화형은 사용자가 게이트 — 매치 시 ⚠ 경고만 표시
...
│   ├── gate.rs      (신규) TtyApprover (y/N 프롬프트, ⚠ 경고)
...
```rust
        ...
        while turns < self.max_turns {
            ...
            // 반복 감지 (스펙 §3). finish는 위에서 이미 return/continue 했음
...
- Produces:
  - `ToolCtx { root: PathBuf, cancel: Arc<AtomicBool>, command_timeout: Duration }` + `ToolCtx::new(root: PathBuf) -> Self` (cancel=false, timeout=60s)
...
`Tool` 사용부의 트레이트 객체를 전부 `Box<dyn Tool + Send + Sync>`로 (Registry 필드, `new`, `get` 반환은 `&(dyn Tool + Send + Sync)`... `get`은 `Option<&(dyn Tool)>` 유지 가능 — 반환에는 바운드 불필요). 기존 `ToolCtx { root: ... }` 리터럴(각 툴 테스트, repl, main)은 `ToolCtx::new(...)`로 일괄 교체.
...
`src/main.rs` oneshot도 `ToolCtx::new(root)` 사용 (취소 배선은 REPL만 — -p의 Ctrl+C는 프로세스 종료).
...
fn spawn_reader(r: Option<impl Read + Send + 'static>) -> std::thread::JoinHandle<Vec<u8>> {
...
impl Tool for RunCommand {
    ...
    fn run(&self, args: &serde_json::Value, ctx: &ToolCtx) -> Result<String, ToolError> {
        ...
        let stdout = decode(&out_h.join().unwrap_or_default());
        let stderr = decode(&err_h.join().unwrap_or_default());
...
`src/ui/status.rs` tests — `action_lines_are_compact`에 추가:
...
pub struct TtyApprover<'a> {
    ...
    /// 대화형에서는 차단하지 않고 ⚠ 경고만 (스펙 §5 — 사용자가 게이트)
    pub deny: &'a [Regex],
}
...
(`Regex`는 `Clone`이라 `deny.to_vec()` 가능.) `/help`에 두 줄 추가:
...
- Produces (`src/session.rs`):
  ...
  - `Session::{snapshot() -> Snapshot, rollback(Snapshot)}`
  - `Session::pack(input_budget_tokens: usize)` — 저장 히스토리 변형
  - `session::tool_result_message(tool: &str, body: &str) -> ChatMessage` (agent/mod.rs에서 **이동**)
...
impl Session {
    ...
    pub fn snapshot(&self) -> Snapshot {
        Snapshot { len: self.messages.len(), tail: self.messages.last().cloned() }
    }

    /// 실패/중단 롤백 — 길이 절단 + 꼬리 원복 (병합 경로 대응, M2 repl::rollback 이동)
    pub fn rollback(&mut self, snap: Snapshot) {
        self.messages.truncate(snap.len);
        if let (Some(slot), Some(orig)) = (self.messages.last_mut(), snap.tail) {
            *slot = orig;
        }
    }
...
- Architecture 추가 요점 (간결히):
  ...
  - `--auto` deny patterns: default cross-platform list in config, blocked only in auto mode (interactive shows ⚠ warning)
