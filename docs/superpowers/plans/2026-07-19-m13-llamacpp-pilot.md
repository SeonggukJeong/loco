...
`test_agent()`에 해당하는 기존 헬퍼가 없으면, 같은 모듈의 다른 테스트가 `Agent`를 만드는 방식을 그대로 복사해 쓴다. **헬퍼 이름을 지어내지 말 것** — 기존 코드를 읽고 맞춘다.
...
`src/eval/mod.rs`의 `judge` 시그니처에 인자를 추가한다(이미 `#[allow(clippy::too_many_arguments)]`가 붙어 있다):
...
- **배치 조건**: `cargo run -- eval tasks --repeats 3` (36런), ornith@8K, seed 0, `scripts/serve.sh` 핀 적용, `.loco/config.toml`의 정확한 내용을 그대로 전재
- **대조**: `20260718T222824Z` (33/36, 엄격 32)
- **결함 하한 (판정 규칙보다 먼저)**: 앵커가 **< 27/36**이면 앵커로 기록하지 않고 마일스톤 정지·진단
- **기계 검사 3종** (전건 통과해야 앵커로 인정):
  - `parse_fail_first(총) == 0` (T4)
  - `report.json`의 모든 런에서 `schema_fallback == false` (T3)
  - 서버 기동 로그의 `n_ctx_slot == 8192`
...
- **관측 항목(판정 아님)**: `finish_reason == "length"` 턴 수(§3-2 사고 토큰 예산 잠식), `empty_test_note`, 상태선 렌더 수
...
python3 -c "import os,sys; os.setsid(); os.execvp(sys.argv[1], sys.argv[1:])" \
  cargo run --release -- eval tasks --repeats 3 > /tmp/m13-anchor.log 2>&1 &
```
...
- [ ] **Step 6: `finish_reason: length` 관측 (판정 아님)**

스펙 §3-2의 1순위 관측 항목이다. 사고 토큰이 예산을 잠식하면 content가 빈 채 length로 끝난다:
...
**Files:**
- Modify: `src/agent/status_note.rs:15` (케이던스), `src/agent/status_note.rs:94-99` (무뮤테이션 렌더)
...
Run: `cargo test --lib agent::status_note 2>&1 | tail -25`
Expected: 3건 중 최소 2건 FAIL — 케이던스 테스트는 `turn 3`에서, 검증 줄 테스트는 그 문자열이 없어서

기존 테스트 `zero_mutation_cadence_fires_at_5_10_15_20_only`(`src/agent/status_note.rs:181`)도 이제 실패한다 — **그 테스트는 삭제하고 새 것으로 대체한다**(같은 사실의 갱신이므로 중복 유지는 잘못이다).
...
```bash
git add src/agent/status_note.rs
...
**⚠️ 위 heredoc의 셸 변수 보간은 diff에 따옴표·백슬래시가 있으면 깨진다.** Step 2에서 반드시 실제 diff로 검증하고, 깨지면 파이썬에 환경변수로 넘기는 방식(`DIFF="$DIFF" python3 - ... <<'PYEOF'` + `os.environ`)으로 바꾼다. **`'PYEOF'`(따옴표)로 바꾸면 보간이 꺼지므로 환경변수 방식과 짝을 맞춰야 한다.**

- [ ] **Step 2: 따옴표·백슬래시가 든 diff로 검증**

```bash
chmod +x scripts/pilot.sh
mkdir -p /tmp/pilot-test && cd /tmp/pilot-test && git init -q 2>/dev/null || true
printf 'let s = "he said \\"hi\\"";\n' > a.rs && git add -A && git commit -qm init
printf 'let s = "he said \\"bye\\"";\nlet t = '"'"'x'"'"';\n' > a.rs
cd /Users/sgj/develop/loco
```

이 상태에서 `pilot.sh`를 `LOCO_BIN=true`(즉시 종료하는 더미)로 돌려 원장 한 행이 **유효한 JSON**인지 확인한다:

```bash
cd /tmp/pilot-test
LOCO_BIN=true PILOT_LEDGER=/tmp/pilot-test/ledger.jsonl /Users/sgj/develop/loco/scripts/pilot.sh
python3 -c "
import json
for l in open('/tmp/pilot-test/ledger.jsonl'):
    r=json.loads(l); print('OK — diff 길이', len(r['diff']), '| verdict', r['verdict'])
"
cd /Users/sgj/develop/loco
```

Expected: `OK — diff 길이 <0보다 큼>` — **JSON 파싱 예외가 나면 Step 1의 경고대로 환경변수 방식으로 고친다.**
...
**플랜 작성 중 자체 적발한 오류 1건**: T7 Step 1의 테스트가 처음에 `StatusNote::default()`와 `record_command(Some("101"), Some(&TestSummary{...}))`를 썼는데, 실제 API는 `StatusNote::new()`와 `record_command_result(Option<String>, Option<TestSummary>)`(값 전달)였다. 실측으로 확인해 수정했다 — 그대로 넘겼으면 T7이 컴파일 실패로 시작했을 것이다.

**여전히 구현자 확인이 필요한 것**: T3 Step 1의 `test_agent()` 헬퍼는 존재를 확인하지 않았다. `src/agent/mod.rs`의 기존 테스트가 `Agent`를 만드는 방식을 읽고 맞출 것 — 이름을 지어내지 말 것.
