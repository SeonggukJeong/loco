...
- **커밋**: Conventional Commits (제목 한국어 허용), 태스크당 최소 1커밋. **예외: T11**은 loco 레포에 커밋하지 않는다 — 산출물인 파일럿 원장 JSONL에 개인 코드 diff가 들어가므로 레포 밖에 둔다(T11 Files 참조). T11의 결과는 T12가 커밋한다
- **GPU 배치 태스크(T6·T8)는 GPU 시간을 쓴다** — 아래 "측정 규율" 준수. **T11은 GPU 배치가 아니다**(스펙 §4: "사용자 시간, GPU 배치 아님") — 측정 규율은 T11에 적용되지 않으며, 특히 규율 6(데몬화)을 `pilot.sh`에 적용하면 대화형 `read -r` 프롬프트가 깨진다

### 측정 규율 (T6·T8 필수 — T11 비적용)
...
print("적용한 안정 집합:", STABLE)   # 사전등록 목록과의 드리프트를 산출물에 남긴다
...
- Modify: `docs/experiments/2026-07-19-llamacpp-anchor/report.md` (`verify_*` 비교가능성 각주 — Step 7. `baselines.md`가 아니다: 그 M13 절은 T12가 만든다)
...
4. **디스패치 경로의 `!stop` 가드**(`src/agent/mod.rs:499` 근방)에서 `!stop`을 뺀다 → Step 2에서 고친 `repetition_stop_still_fires_with_status_note_active`가 FAIL해야 함

**변이 4가 이 태스크에서 가장 중요하다.** Step 2가 한 문단을 들여 지킨 불변식이 바로 그것인데, 이 변이를 안 돌리면 Step 6이 정작 가장 위험한 것만 빼고 전부 검사하는 꼴이 된다.

각 변이가 **실제로 빨간불을 만드는지** 확인하고 되돌린다. 죽지 않는 변이가 있으면 그 테스트가 공허하다는 뜻이다 — 테스트를 보강할 것.

⚠️ **기지의 커버리지 공백(M13이 만든 것이 아님, 고치지 않아도 됨)**: `!stop` 가드는 **두 곳**에 있다 — 디스패치 경로(`mod.rs:499` 근방)와 거부 경로(`mod.rs:399` 근방). 이 테스트는 디스패치 경로만 탄다. **거부 경로의 `!stop`을 통째로 지워도 전체 테스트가 초록불이다**(플랜 리뷰 실측: `365 passed; 0 failed`). 변이 4를 넣을 때 `:399`가 아니라 `:499`를 고쳐야 죽는다. 이 공백을 Step 7의 각주에 한 줄 기록해 두어, 나중에 읽는 사람이 두 가드가 다 핀으로 박혀 있다고 오해하지 않게 한다.
...
각주를 **`docs/experiments/2026-07-19-llamacpp-anchor/report.md`**에 남긴다. `docs/baselines.md`가 아니다 — 그 파일의 M13 절은 T12가 만들며 T7 시점에는 존재하지 않는다(순서: T6 → T7 → T8 → … → T12). T6 Step 7이 만든 `report.md`는 T7 시점에 이미 있다. **T12 Step 2가 이 각주를 `docs/baselines.md`의 M13 절로 승계한다.**

M12가 `sr_error`에 대해 같은 성격의 각주를 남긴 전례를 따른다:
...
git add src/agent/status_note.rs src/agent/mod.rs \
        docs/experiments/2026-07-19-llamacpp-anchor/report.md
...
experiments/.../report.md에 비교가능성 각주(T12가 baselines.md로 승계).
거부 경로 !stop 가드의 기지 커버리지 공백도 함께 기록."
```
...
{"session_id":"20260719T140000Z","repo":"/path/to/repo","start_rev":"abc1234",
 ...
 "duration_secs":312,"verdict":"수정해서 씀","reason":"S/R 루프로 두 번 헤맴"}
```
...
**판정 기준은 "유효한 JSON이고 길이가 0이 아니다"가 아니다.** 그 기준은 손상된 diff도 통과시킨다(실측 확인: `\"` → `"`로 소실돼도 JSON은 유효하고 길이도 0이 아니다). **기록된 diff가 `git diff` 출력과 일치하는지**를 본다 — 후행 개행 하나만 정규화하고, 나머지는 바이트 동일을 요구한다.
...
**첫 답으로 `y`를 넣는다** — 픽스처가 워킹트리를 **일부러** 더럽힌 상태(그래야 검사할 diff가 생긴다)라 `pilot.sh`의 더러운-워킹트리 확인 프롬프트가 먼저 뜬다. 안 넣으면 첫 답 `bugfix`가 그 프롬프트에 먹혀 스크립트가 exit 1로 죽고, 원장 파일이 아예 안 생긴다.
...
# 셸의 $( ) 는 후행 개행을 전부 제거한다. 올바른 스크립트로도 raw 바이트 동일은
# 성립할 수 없으므로 그 한 가지만 정규화하고 비교한다 — 우리가 잡으려는 손상
# (\" -> ", \n -> 개행, """ 절단)은 전부 내부 바이트를 바꾸므로 이 정규화를
# 거쳐도 여전히 불일치로 잡힌다.
print("동일(후행 개행 정규화):", row["diff"] == expected.rstrip("\n"))
print("기록 길이", len(row["diff"]), "| 기대 길이", len(expected.rstrip("\n")))
if row["diff"] != expected.rstrip("\n"):
    print("!!! 손상됨 — Step 1의 환경변수 방식이 제대로 적용됐는지 확인할 것")
EOF
cd /Users/sgj/develop/loco
```

Expected: `동일(후행 개행 정규화): True`
...
**⚠️ 사전 스캔 판정 반영 (사용자 승인 2026-07-19):** 스펙 §4-2는 JSONL을 고른 근거로 "과정 지표는 `exp_metrics.py` 확장으로 재사용된다"고 적었는데, 이 플랜의 T10은 독립 스크립트다. 그대로 두면 검증된 마커 상수와 분리된 **두 번째 분류기**가 생겨 드리프트한다. 결정: **집계 로직은 독립 유지하되, 마커 판정 상수는 `exp_metrics.py`에서 가져다 쓴다.** 아래 `classify()`가 부분문자열을 자체 리터럴로 들고 있으면 안 된다 — `scripts/exp_metrics.py`의 기존 상수(`MARKS`, `BADARGS_KEY_PREFIX` 등)를 import해 사용하고, 필요한 상수가 그쪽에 없으면 `exp_metrics.py`에 상수로 신설한 뒤 양쪽이 그 하나를 참조하게 한다(리터럴 복제 금지). `exp_metrics.py`는 stdlib 전용 단일 파일이므로 `sys.path`에 `scripts/`를 넣어 import하면 된다. import 실패 시 조용한 폴백을 두지 말 것 — 큰 소리로 실패해야 한다.

**⚠️ 구현 중 인터페이스가 바뀌었다 (T10 리뷰 수선 2, `1d023c0`).** 아래 코드 블록의
`python3 scripts/pilot_tally.py <ledger.jsonl> <repo-path>` 는 **옛 형식**이다. 파일럿이
4개 레포에 걸치도록 확장되면서, 레포 경로 하나를 모든 행에 일괄 적용하면 다른 레포
소속 행은 `git grep`이 전부 미스 처리돼 생존율이 아무 표시 없이 과소 계상됐다. 레포는
이제 각 행 자신의 `repo` 필드에서 읽으며 **CLI 인자는 원장 하나뿐**이다:
`python3 scripts/pilot_tally.py <ledger.jsonl>`. 아래 블록은 착수 시점 기록으로 남긴다.
...
```bash
python3 scripts/pilot_tally.py <원장>
```
...
```bash
python3 scripts/pilot_tally.py <원장> > /tmp/pilot-summary.txt
...
- **T7 Step 7의 `verify_*` 비교가능성 각주를 `report.md`에서 승계**한다 — M13 절이 여기서 처음 생기므로 그 각주의 최종 거처는 이 문서다. 거부 경로 `!stop` 가드의 커버리지 공백 기록도 함께 옮긴다
- **합성 평가 통과율 ↔ 실사용 채택률의 괴리**를 쓸 때, 배치는 디버그·파일럿은 릴리스 빌드라는 점을 한 절 넣는다 — 빌드 프로파일은 loco 자신의 지연만 바꾸고 모델 출력·결정은 못 바꾸므로 괴리와 무관하지만, 나중에 읽는 사람이 그걸 다시 유도하지 않아도 되게 한다
...
| N1 | Nit | 미채택(2R에서 리뷰어 동의) — 안정 집합 6개가 3곳에 하드코딩된 것은 드리프트 위험이나 사전등록이 단일 진실이고 스니펫은 사본이다. 다만 2R 제안대로 T6·T8 스니펫이 **적용한 목록을 출력**하게 해 드리프트가 산출물에 남게 했다 |

## 플랜 리뷰 2R 처분 (`463a112` 대상, Ready=Yes 조건부)

2R은 T9 스크립트를 축자 추출해 `sh -n`·export 감사·적대적 픽스처 실행을 했고, T7의 위험 테스트에 대해 **제안한 대체 단언을 뮤테이션 테스트로 검증**했다(가드 있으면 통과, 빼면 실패 — 공허하지 않음). 컨트롤러가 R1을 독립 재현했다.

| # | 처분 |
|---|---|
| R1 (Important) | 수용 — **강화한 기준이 통과 불가능했다.** 셸 `$( )`가 후행 개행을 전부 제거하므로 올바른 스크립트로도 raw 바이트 동일이 성립하지 않는다(컨트롤러 재현: 113 vs 112바이트, 차이는 개행 하나). "통과할 때까지 T9를 끝내지 말 것"과 겹쳐 **만족 불가능한 차단 게이트**가 됐고, 실패 시 예상 반응은 기준 약화 — 강화의 목적을 정확히 뒤집는다. 후행 개행만 정규화(내부 바이트를 바꾸는 모든 손상은 여전히 잡힘) |
| R2 (Important) | 수용 — Step 2 픽스처가 워킹트리를 일부러 더럽히므로 더러운-워킹트리 프롬프트가 먼저 뜨고, 첫 답 `bugfix`가 거기 먹혀 스크립트가 exit 1로 죽는다. `y\n` 선행 + 사유 명시 |
| R3 (Important) | 수용 — T7 Step 7이 T12가 만드는 `baselines.md` M13 절에 쓰라고 했다. T7 시점에 존재하는 `report.md`로 대상 변경, T12가 승계. 두 태스크의 문서 소유권 충돌도 함께 해소 |
| M1 (Minor) | 수용 — Step 6 뮤테이션 목록에 **`!stop` 가드 제거**를 4번으로 추가. Step 2가 한 문단을 들여 지킨 불변식이 정작 뮤테이션 대상에서 빠져 있었다 |
| M2 (Minor) | 수용(기록만) — `!stop` 가드는 두 곳인데 **거부 경로(`mod.rs:399`)는 지워도 전체 초록불**이다(리뷰어 실측). M13이 만든 공백이 아니므로 고치지 않되, 변이 4가 `:499`를 겨냥해야 함과 이 공백을 각주에 기록 |
| M3 (Minor) | 수용 — 원장 스키마 예시의 `duration_secs`를 `312.4`에서 `312`로(코드는 `int`) |
| T11 `--release` | 리뷰어 확인 — T12가 파일럿과 배치 수치를 like-for-like로 비교하는 곳이 없고, 빌드 프로파일은 loco 자신의 지연만 바꿀 뿐 모델 출력·결정을 못 바꾼다. 추론 성립 |
