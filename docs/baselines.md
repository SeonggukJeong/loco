...
실패를 좌우한 것은 소수(#2·#7·#8·#9)뿐이고 나머지(특히 #10 갓파일·#11 재수출
사슬)는 `grep`의 정밀한 질의(`fn <함수명>` 패턴)에 9런 전부 무력화됐다 —
find-definition-large의 실패는 함정이 아니라 종료 규율(finish 인자 누락·강박적
재검증)에서 나왔다. 가장 크고 새로운 발견은 스펙 §8 백로그에 없던
`edit_file` 자기-버그(`search`와 `replace`에 동일한 수정 전 텍스트를 넣는
실행 결함) — ornith 전용 패턴으로 27런 중 9런·37회 발생하며, 8K에서는 직접적
실패 원인(반복정지·파일 손상)으로, 32K에서는 `write_file`/`sed`/`python3`
