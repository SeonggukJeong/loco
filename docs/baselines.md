...
## M14 — 비교가능성 각주 (정직한 검증 신호 II)

스펙: `docs/superpowers/specs/2026-07-20-m14-honest-verification-ii-design.md`.
이 절은 골격이다 — **측정 전에** 먼저 적는다. §8의 회귀 게이트 측정은 아직
수행되지 않았다(사전등록은 별도 태스크, 측정 결과는 그 뒤 다른 태스크가
채운다). 측정 결과를 이 절 아래에 채울 때도 아래 각주는 지우지 말 것 —
데이터를 본 뒤에 각주를 쓰면 각주가 아니라 사후 변명이 된다.
