...
- **F5**: 과제는 *"오류 메시지에 어떤 입력이 문제였는지 더 분명히 담아줘"*였는데
  원본이 **이미 입력을 보간 중이었고**(`anyhow!("'{}' is not …", s)`), 모델은
  원인 힌트(`, or is not a valid UID`)를 덧붙였다. 그룹명 갈래
  (`is not a recognized group name`)는 미수정. 요청받은 변경을 하지 않았다.
...
**유형을 맞추면 훨씬 선명하다.** `tasks/` 12과제 중 **파일 경로를 주지 않는
과제는 `find-definition` 하나**다(*"이 프로젝트에서 `area` 함수가 정의된 파일을
찾아…"* — 픽스처는 `.rs` 4파일, 약 10줄). 파일럿에서 같은 형태(과제문이 파일
경로를 지정하지 않아 모델이 어디를 볼지 스스로 골라야 하는 과제)는 7세션이다.

| | 규모 | 결과 |
|---|---|---|
| 합성 `find-definition` | 4파일 / ~10줄 | 앵커 3/3 + 게이트 3/3 = **6/6** |
| 실사용 파일 경로 미지정 7세션 | 2,820 ~ 64,723줄 | **0/7** (Z1·F1·F3·R1·R2·J1·J5, 전건 버림) |

파일 경로가 **명시된** 13세션은 성공 7이다. 사전등록 축인 과제 유형으로 보면
`explore` 0/5인데, 위 축은 그 상위 집합이고(feature 2건 추가) 기전을 함께 준다 —
실패한 것은 "탐색"이라는 유형 라벨이 아니라 **모델이 어디를 볼지 스스로 골라야
하는 조건**이다.
...
| `SR_CORRECTION` | 4회 / 3세션 | 4회 전부 다음 1~2턴 내 편집 성공으로 이어졌다 |
