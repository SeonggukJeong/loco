...
무뮤테이션 검증 줄 렌더는 T7 이후에만 존재하므로 앵커(T7 이전)에서는 구조적으로 0이고,
그것이 아래 `verify_*` 비교불가 각주의 이유다.
...
**★ 그리고 앵커 배치의 유일한 실패가 이 축에 있다** — `rename-function` 2/3
(`repetition_stop`). 즉 합성 세트가 이 축을 **전혀 못 재는 것이 아니라, 재긴
재는데 실패가 한 건 나올 만큼만 약하게 잰다.** 이 사실은 대비를 약화시키지
않고 오히려 M14의 범위 결정(아래 후보 2-1)에 실증적 근거를 준다.
...
| `SR_CORRECTION` | 4회 / 3세션 | 4회 전부 다음 1~2턴 내 **편집 디스패치 성공**으로 이어졌다. ⚠ **과제 성공이 아니다** — 3세션 중 사용자 채택은 R5 하나뿐이고(F5는 전제 부정확, J4는 버림), 그 R5의 해당 편집이 바로 아래 "커버리지 후퇴"를 만든 편집이다 |
