...
- **상태: 초안 (승인 대기)** — PROTOCOL 1. 사용자 승인 커밋 전에는 T24 GPU 배치를 시작하지 않는다.
  승인 시 이 행만 `승인됨(YYYY-MM-DD)`로 바꾸고, 조건·표본·판정·중단·예산은 불변으로 둔다
  (M11·M12·M13·M14 전례: 전언 승인은 문서 승인을 대체하지 못한다).
...
- [ ] 위 조건·표본(N=17)·로드 **37601**·`r_obs` **1.2587**·실격 **≥13**·예산 **51런/12h**·
      재측정 공약을 데이터 없이 확정한다
- [ ] 상태 행을 `승인됨(날짜)`로 바꾸는 커밋이 승인의 성립 근거다
- 승인자 / 날짜:

**⛔ 승인 전 T24 착수 금지 (PROTOCOL 1).**
