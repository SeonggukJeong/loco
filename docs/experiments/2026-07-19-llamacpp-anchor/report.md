...
- **디버그 빌드** — 명령은 앵커와 동일한 `cargo run -- eval tasks --repeats 3 --seed 0`
  이고, 배치 로그에 `Finished \`dev\` profile [unoptimized + debuginfo]`가 남았다.
  사전등록 §2-3은 이 조건을 **두 배치 공통**으로 등록했다 — 빌드 프로파일은
  `report.json`에 남지 않아 사후에 발견할 수단이 없기 때문이며, 따라서 배치마다
  명시적으로 재확인해 기록한다(이 줄은 T8 리뷰 지적으로 추가됨: 최초 작성 시
  T6 절에만 있고 T8 절에 빠져 있었다)

**T7 diff 사람 정독** (사전등록 §1 — 파일 범위 제약은 내용을 보증하지 않으므로
필수 절차). 확인자: M13 SDD 컨트롤러 세션(Claude Code, 세션
`af307389-c5b5-40be-9e3b-9cb0746b0342`) — 사람 사용자 본인이 아니라 사용자를
대신해 수행한 에이전트 세션이며, 이 구분을 감추지 않는다. 확인 시각:
2026-07-19 09:30Z 경(배치 시작 09:32:54Z 이전). 아래 명령 출력은 재현 가능하다.
...
| 지표 | 게이트 | 앵커 | 비고 |
|---|---|---|---|
| 빈-content 턴 (`"(empty)"`) | **7** | 6 | 대조 배치는 3. 대리 지표 — §4의 한계 설명이 그대로 적용된다 |
| `sr_error` | 31 | 41 | |
| `sr_correction` | 7 | 14 | |
...
빈-content 턴은 세 배치에서 대조 3 → 앵커 6 → 게이트 7로 단조 증가했다.
**판정에 쓰지 않으며 방향을 주장하지도 않는다** — 절대 수가 작고(배치당 36런),
앵커→게이트 사이의 코드 변경(T7)은 상태선 렌더러일 뿐 토큰 예산과 무관하다.
다만 대조(LM Studio)에서 llama.cpp 두 배치로 넘어오며 배가된 것은 스택 차이일
가능성이 있어 **M14 관측 대상으로 남긴다**. (T8 리뷰 지적으로 추가된 행이다 —
최초 작성 시 이 지표가 게이트 절에서 빠져 있어, 세 배치 중 가장 높은 값이
표에 드러나지 않았다.)
