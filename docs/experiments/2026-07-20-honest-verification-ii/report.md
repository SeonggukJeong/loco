...
**의의**: M13이 20세션에서 0회로 "미관측"이라 기록한 규칙이 **M12
회귀 게이트 배치(1회)와 이 배치(2회)를 합쳐 총 3회 실발화**했다.
"불필요"가 아니었음이 뒷받침된다 — CLAUDE.md의 `ARGS_TOOL_SWITCH_NOTE`
"미관측" 서술은 이 배치로 갱신 대상이다. 다만 n이 작아 발화율은
추정하지 않는다.
