...
| 완고 루프 발생 런의 종결 전환율 | 85.7%(기준선=자연 전환율) | 100% | 100% (N=3, 소표본 — §3 전수 나열) | ②=③ 동률 |
...
|---|---|---|---|
| 1 | 게이트: `cargo test` + `cargo clippy --all-targets -- -D warnings` + `eval tasks --verify` 12/12 + `eval tasks-large --verify` 3/3 (모든 암 브랜치에서) | **충족** | §0 배치 전 게이트(전 배치 12/12·3/3) + 병합 후 재확인(cargo test 293·clippy 0·verify 12/12+3/3, 위 "배치 ↔ 커밋 ↔ 스탬프" 절 참고). cargo test·clippy는 암 브랜치별 구현 게이트(Task 6·7, 각 커밋 전 수행)에서도 통과했다. |
...
| 3 | 행동 지표(주): 승자 암에서 ①S/R발 반복정지 0건 ②완고 루프 발생 런의 종결 전환(수정 성공 또는 유효 finish 도달) ③오류당 2시도 내 회복률이 기준선 대비 상승 — 소표본 규칙(<3런은 전수+방향) 승계 | **부분 충족(각주 참조)** | 승자 암③: ① sr발 반복정지 0건(8K §2-1, 32K §7-3 모두 0) ② 종결 전환율 100%(8K N=3·32K N=6, §2-1·§7-3) ③ 오류당 2시도 내 회복률 42.9%(①)→66.7%(③), 32K에서 80.0%로 추가 상승(§7-5) — 기준선 대비 전 항목 상승 |
...
> **각주 (기준 3②)**: 측정의 운영화는 스펙 §2 문언과 두 가지가 다르다 — (a) 스펙의 완고
> 루프는 "동일 파일 S/R 3회+"이나 `exp_metrics.py`는 파일 귀속이 없어 **런 수준
> sr_error≥3**으로 근사했다, (b) 스펙의 종결 전환은 "수정 성공 또는 유효 finish 도달"이나
> 측정된 전환율은 **1−(sr발 반복정지 비율)**이다. 과제 수준 독법으로는 8K 암③의 완고 런
> 3개(update-vat-rate-3/6/8)가 모두 max_turns·passed=False(0/3)이고, 편집 수준 독법(막힌
> 편집이 결국 착지: sr_recovered 4/6·4/7·2/5)으로만 충족이다. 승자 판정은 이 지표가 아니라
> 회복률 우세(§8-1)에 기반하므로 어느 독법에서도 불변이다. 이는 부분적으로 스펙 결함(§2
> 문언이 §7-3 exp_metrics 열 사양과 미정합)이다.
