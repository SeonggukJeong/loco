...
M12는 새 개입의 효과를 입증하는 실험이 아니라, 하네스 자신의 거짓
초록불 결함을 수선하는 **회귀 게이트**다(uv 통과율 개선은 목표가 아님).
libtest 요약을 실제로 파싱해(합산 파서, `None` 폴백) 상태선의 verification
문구를 exit code 하나가 아니라 5규칙(실패명 인용·0-테스트 무효화·exit
교차검증 등)으로 접지하고, FINISH_NUDGE/VERIFY_NUDGE가 "0개 테스트 실행,
exit 0"을 성공 검증으로 오인해 무장하던 두 거짓 초록불을 봉쇄했다. 이미
검증된 M9/M10 개입(S/R 교정, 온도 섭동)의 도달률도 넓혔다 — S/R 카운터를
연속 전용에서 파일별 누적으로 확장(발화 상한 3회로 풍선효과 방지), 온도
섭동 트리거에 BadArgs missing-field 연속 스트릭을 추가, args 안에 잘못
들어간 `tool` 키를 디스패치 직전에 정규화하는 salvage 역방향 규칙을
신설했다. 사전등록 회귀 게이트(tasks/ 스포트 36런)는 **33/36으로 임계값
(≥33/36)에 정확히, 재측정 없이 1회로 통과**했다 — 통과 여유가 없다는
사실 자체를 문서에 그대로 기록했다. 배치 후 관측된 이상 징후 3건(신규
`stop_cause=sr` 반복정지 1건, S/R 회복률 하락, 거짓 성공 finish 1건)은
전용 법의학으로 전수 귀속 분석한 결과 셋 다 M12 신규 장치의 실패가
아님을 확인했다(라벨만 바뀐 기존 실패, 단일 런에 갇힌 통계 왜곡, 뮤테이션
0건 상태의 자신 있는 finish — M12 이전부터 있던 별개 문제). 반대로
`update-vat-rate-8`에서는 기함이 실전 트랜스크립트에서 진짜 거짓 초록불
(`0 tests run, exit 0`)을 잡아 모델의 다음 행동을 바꾼 사례도 확인됐다.
상세는 `docs/baselines.md` "M12 실험 — 회귀 게이트" 절과
`docs/experiments/2026-07-18-honest-harness/report.md`.
