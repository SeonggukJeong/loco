...
## 프로젝트 상태: M6 완료 · 저사양 대형모델 방향 탐색 (2026-07-16)

M6(판정·평가 신뢰성 개편)을 재개·구현 완료했다 — 메타테스트 `eval --verify`(과제별
변별성+해결가능성 게이트), answer 판정 정규화 사다리, 이중 리포트(통과/엄격/거짓 성공
finish), 에이전트 코드 동결 하에 v2 기준선 재측정까지. 판정기가 바뀌어 v1 계열(기준선·M5)과
직접 비교는 불가하다 — 상세는 `docs/baselines.md`.

방향 전환("저사양 하드웨어에서 대형 모델 구동")의 첫 실측도 병행했다: **Qwen3 계열 9B
(Ornith 1.0)가 4B와 대등한 속도로 12과제 94.4%(엄격 94.4%, 거짓 성공 finish 0)** — 4B 두
모델(gemma·qwen)이 공통으로 막히던 count-usages·multiline-string-edit까지 돌파했다. 소형모델
스캐폴딩(loco 코어)은 유효하다고 보고, 다음은 M7 스코핑(품질 마무리 vs 대형·큰-컨텍스트 방향).
...
| ornith-1.0-9b (탐색, 기준선 아님) | 94.4% (34/36) | 94.4% (34/36) | 0 |

측정 조건: 모델 단독 로드 ctx 8192, `max_output_tokens = 4096`, seed 0, `--repeats 3`
(과제 12 × 3반복 = 36런), 하네스 커밋 `4cb7325`.
