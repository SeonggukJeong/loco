...
## 모델 비노출 원칙 (베이스 테스트 주석, 2026-07-17 리워드로 확정)
베이스(공통) 테스트가 특정 값을 단정하지 않는 이유(위 함정 대장의 판정 신호
설계)는 이 README에만 적는다 — fixture/ 안의 doc 주석은 모델에게 그대로
노출되므로, 그 이유를 "판정"/"과제"/"금지 구역"/"~을 심는다" 같은 평가자
어휘로 적으면 grep 한 방에 채점 로직이 새어 나간다(2026-07-17 최종 리뷰에서
27런 중 10런 노출 확인 — 상세는 `docs/research/2026-07-17-m8-failure-analysis.md`
§5). fixture/ 안 doc 주석은 반드시 인다월드(in-world) 어휘로만 쓸 것 — 예:
"이 값은 설정/정책에 따라 바뀔 수 있어 여기서는 단정하지 않는다, 계산 로직
자신의 테스트가 값 검증을 맡는다" 같이, 실제 엔지니어가 남길 법한 이유만
적는다. 이 절 이후로 fixture/ 주석에 평가자 어휘가 다시 들어가면 아래
린트가 잡아야 한다(회귀 방지).

## 드리프트 방지 절차 (베이스 수정 시)
베이스 = find-definition-large/fixture (판정 파일 tests/check_*.rs 제외).
0. 평가 메타 어휘 린트 — fixture/ 주석에 채점 의도가 새지 않았는지 확인
   (판정 파일 tests/check_*.rs는 "과제 판정" 같은 표현이 플랜 고정 텍스트라
   예외):
   ```
   grep -rn "함정\|오판\|일부러\|심는\|심은\|금지 구역\|판정 신호\|과제[0-9 ]" tasks-large/*/fixture/ --include='*.rs' | grep -v 'tests/check_'
   ```
   빈 출력이어야 한다. 노이즈가 심해지면(예: "과제" 단어가 인다월드 텍스트에
   섞여 들어가는 경우) 좁히되, 왜 좁혔는지 이 절에 기록할 것.
...
5. 0번 린트를 다시 돌려 rsync 후에도 빈 출력인지 재확인
