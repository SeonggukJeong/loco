...
//!
//! 함정5 짝A: 아래 `MAX_RETRY`는 inv-store 크레이트의 `RETRY_LIMIT`
//! (inv-store/src/retry.rs)과 이름은 다르지만 같은 값(3)을 갖는 짝이다.
//! 두 크레이트는 서로를 참조하지 않으므로, "재시도 한도"를 grep 한 줄로
//! 찾으려 하면 어느 한쪽만 걸리고 나머지 절반의 재시도 로직을 놓치게 된다.
