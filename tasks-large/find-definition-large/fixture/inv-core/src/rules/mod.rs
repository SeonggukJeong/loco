...
//!
//! 함정10: 이 파일은 "갓파일"이다. 원래는 pricing/allocation처럼 서브모듈로
//! 쪼갤 계획이었으나, 새 규칙이 필요할 때마다 여기 계속 덧붙여지며 리팩터링이
//! 미뤄진 채 누적되었다 — 사내에서 실제로 자주 벌어지는 패턴이다. 재고 임계값
//! 계산(`restock_threshold`)과 창고 등급(`WarehouseGrade`) 정의는 이 파일의
//! 맨 아래에 있다(함정11 재수출문은 `inventory.rs`를 참고).
...
// ---------------------------------------------------------------------
// 섹션 12: 창고 등급 및 재고 임계값 (핀 고정 — 함정10 위치, 함정11 대상)
// ---------------------------------------------------------------------
//
// 위 규칙 함수 더미에 묻혀 찾기 어려운 위치에 일부러 배치되어 있다.
// `inventory.rs`가 아래 두 항목을 재수출한다(함정11) — 재수출문만 보고
// "정의가 inventory.rs에 있다"고 오판하지 않도록 주의.
